...
import (
	...
	orderer "github.com/hyperledger/fabric-sdk-go/fabric-client/pkg/orderer"
...
func GetChain(client api.Client, chainID string) (api.Chain, error) {

	chain, err := client.NewChain(chainID)
	if err != nil {
		return nil, fmt.Errorf("NewChain return error: %v", err)
	}
	orderer, err := orderer.NewOrderer(fmt.Sprintf("%s:%s", config.GetOrdererHost(), config.GetOrdererPort()),
...
func SendInstallCC(client api.Client, chain api.Chain, chainCodeID string, chainCodePath string, chainCodeVersion string, chaincodePackage []byte, targets []api.Peer, deployPath string) error {
...
func SendInstantiateCC(chain api.Chain, chainCodeID string, chainID string, args []string, chaincodePath string, chaincodeVersion string, targets []api.Peer, eventHub api.EventHub) error {
...
func CreateAndJoinChannel(client api.Client, ordererUser api.User, orgUser api.User, chain api.Chain, channelConfig string) error {
