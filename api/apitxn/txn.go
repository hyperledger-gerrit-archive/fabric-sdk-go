...
type ChannelClient interface {
	...
	QueryWithOpts(request QueryRequest, opt QueryOpts) (string, error)

	// ExecuteTx execute transaction
	ExecuteTx(request ExecuteTxRequest) (string, error)

	// ExecuteTxWithOpts allows the user to provide options for transaction execution (sync vs async, etc.)
	ExecuteTxWithOpts(request ExecuteTxRequest, opt ExecuteTxOpts) (string, error)
}
