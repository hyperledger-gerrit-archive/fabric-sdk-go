...
type ChannelEventClient interface {
	...
	RegisterBlockEvent(eventChan chan<- *BlockEvent) (Registration, error)
	...
	// - eventChan is the Go channel to which events are sent. Note that the events should be processed
	//         in a separate Go routine so that the event dispatcher is not blocked.
	RegisterChaincodeEvent(ccID, eventFilter string, eventChan chan<- *CCEvent) (Registration, error)
	...
	// - eventChan is the Go channel to which events are sent. Note that the events should be processed
	//         in a separate Go routine so that the event dispatcher is not blocked.
	RegisterTxStatusEvent(txID string, eventChan chan<- *TxStatusEvent) (Registration, error)
