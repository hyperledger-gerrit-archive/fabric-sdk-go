...
// Manager is responsible for managing organization's identities.
// It requires identity of the CA registrar, for communicating
// with the organization's CA.
type Manager interface {
	Register(request *RegistrationRequest) (string, error)
	Revoke(request *RevocationRequest) (*RevocationResponse, error)
}

// EnrollmentService is a self-service that a user can use
// to enroll and re-enroll itself with a Fabric network.
type EnrollmentService interface {
	// Enroll enrolls a registered user with the org's Fabric CA
	Enroll(enrollmentID string, enrollmentSecret string, attrs ...AttributeRequest) (apicryptosuite.Key, []byte, error)
