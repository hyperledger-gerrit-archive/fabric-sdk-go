...
type FabricCAClient interface {
	...
	Reenroll(user identity.User) (apicryptosuite.Key, []byte, error)
	Register(registrar identity.User, request *RegistrationRequest) (string, error)
	Revoke(registrar identity.User, request *RevocationRequest) (*api.RevocationResponse, error)
}
