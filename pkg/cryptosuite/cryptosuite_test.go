...
func TestCryptoSuite(t *testing.T) {
	...
	keyBytes, err = key.Bytes()
	utils.VerifyEmpty(t, err, "Not supposed to get any error for samplecryptoSuite.GetKey().GetBytes()")
	utils.VerifyTrue(t, string(keyBytes) == mockIdentifier+keyImport, "Unexpected bytes for samplecryptoSuite.GetKey().GetBytes()")

	skiBytes = key.SKI()
	utils.VerifyTrue(t, string(skiBytes) == mockIdentifier+keyImport, "Unexpected bytes for samplecryptoSuite.GetKey().GetSKI()")
