...
// Returns X509 certificate
func (im *FabricCA) Reenroll(user apifabclient.User) (apicryptosuite.Key, []byte, error) {
	if user == nil {
...
func (im *FabricCA) Register(request *sdkApi.RegistrationRequest) (string, error) {
...
func (im *FabricCA) Revoke(request *sdkApi.RevocationRequest) (*sdkApi.RevocationResponse, error) {
	...
	return &sdkApi.RevocationResponse{}, nil
}

func (im *FabricCA) getRegistrar() (apifabclient.User, error) {
	user, err := im.userStore.Load(apifabclient.UserKey{MspID: im.mspID, Name: im.registrar.EnrollID})
	fmt.Printf("=======> getRegistrar 1: %s, error: %s\n", im.registrar.EnrollID+"@"+im.mspID, err)
	if err != nil {
		...
		user, err = im.userStore.Load(apifabclient.UserKey{MspID: im.mspID, Name: im.registrar.EnrollID})
		fmt.Printf("=======> getRegistrar 2: %s, error: %s\n", im.registrar.EnrollID+"@"+im.mspID, err)
	}
	return user, err
}

// createSigningIdentity creates an identity to sign Fabric CA requests with
func (im *FabricCA) createSigningIdentity(user apifabclient.User) (*fabric_ca.Identity, error) {
	// Validate user
	if user == nil {
