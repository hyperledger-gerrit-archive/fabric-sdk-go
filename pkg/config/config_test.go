...
func TestCAConfig(t *testing.T) {
	...
	certfile, err := configImpl.CAClientCertFile("ca-org1")
	...
	keyFile, err := configImpl.CAClientKeyFile("ca-org1")
	...
	sCertFiles, err := configImpl.CAServerCertFiles("ca-org1")
	...
	//Testing CAConfig
	caConfig, err := configImpl.CAConfig("ca-org1")
	if caConfig == nil || err != nil {
		t.Fatal("Get CA Config failed")
	}

	// Test CA KeyStore Path
	if vConfig.GetString("client.credentialStore.path") != configImpl.CAKeyStorePath() {
...
func TestTLSACAConfig(t *testing.T) {
	...
	certFile, _ := configImpl.CAClientCertFile("ca-org1")
	...
	keyFile, _ := configImpl.CAClientKeyFile("ca-org1")
	logger.Debugf("CAClientKeyFile is %s", keyFile)
...
func TestOrdererConfig(t *testing.T) {
	...
	if orderers[0].TlsCACerts.Path != "" {
		...
		t.Fatalf("Orderer %s must have at least a TlsCACerts.Path or TlsCACerts.Pem set", "peerorg1")
	}
}
