...
var configPemTestFilePath = "../../test/fixtures/config/config_test_pem.yaml"
...
func TestInitConfigFromBytesSuccess(t *testing.T) {
	...
	err, cBytes := loadConfigBytesFromFile(t, configTestFilePath)

	// test init config from bytes
	_, err = InitConfigFromBytes(cBytes)
	if err != nil {
		t.Fatalf("Failed to initialize config from bytes array. Error: %s", err)
	}
}

func loadConfigBytesFromFile(t *testing.T, filePath string) (error, []byte) {
...
func TestInitConfigFromBytesEmpty(t *testing.T) {
	// test init config from bytes
	_, err := InitConfigFromBytes([]byte{})
	if err == nil {
...
func TestInitConfigFromBytesWithPem(t *testing.T) {
	//os.Setenv("FABRIC_SDK_CLIENT_LOGGING_LEVEL", "DEBUG")
	// get a config byte for testing
	err, cBytes := loadConfigBytesFromFile(t, configPemTestFilePath)
