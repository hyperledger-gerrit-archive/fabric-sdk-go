...
func (c *Config) TLSClientCerts() ([]tls.Certificate, error) {
	...
	var cb, kb []byte
	if clientConfig.TLSCerts.Client.Cert.Pem != "" {
		cb = []byte(clientConfig.TLSCerts.Client.Cert.Pem)
	} else if clientConfig.TLSCerts.Client.Cert.Path != "" {
		cb, err = loadByteKeyOrCertFromFile(&clientConfig, false)
		if err != nil {
			return nil, errors.Wrapf(err, "Failed to load cert from file path '%s'", clientConfig.TLSCerts.Client.Cert.Path)
		}
	}
