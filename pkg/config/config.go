...
func InitConfigFromBytes(configBytes []byte) (*Config, error) {
	myViper := getNewViper(cmdRoot)

	if len(configBytes) > 0 {
		buf := bytes.NewBuffer(configBytes)
		logger.Debugf("buf Len is %d, Cap is %d: %s", buf.Len(), buf.Cap(), buf)
		// read config from bytes array, but must set ConfigType to yaml
...
func (c *Config) CAServerCertFiles(org string) ([]string, error) {
	...
	certPems = append(certPems, certFileModPath...)
	return certPems, nil
}
...
// Deprecated: use CAClientCert instead
...
func (c *Config) OrderersConfig() ([]apiconfig.OrdererConfig, error) {
	...
	for _, orderer := range config.Orderers {
		if orderer.TLSCACerts.Path != "" {
			...
			errors.New(fmt.Sprintf("Orderer has no certs configured. Make sure TLSCACerts.Pem or TLSCACerts.Path is set for %s", orderer.URL))
		}
