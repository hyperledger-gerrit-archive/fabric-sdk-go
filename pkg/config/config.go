...
func (c *Config) TLSClientCerts() ([]tls.Certificate, error) {
	...
	if clientConfig.TLSCerts.Client.CertPem != "" {
		...
		if err != nil {
			return nil, errors.Wrap(err, fmt.Sprintf("Failed to load cert from file path '%s'", clientConfig.TLSCerts.Client.Certfile))
		}
	}
	...
	if clientConfig.TLSCerts.Client.KeyPem != "" {
		...
		if err != nil {
			return nil, errors.Wrap(err, fmt.Sprintf("Failed to load kry from file path '%s'", clientConfig.TLSCerts.Client.Keyfile))
		}
	}
