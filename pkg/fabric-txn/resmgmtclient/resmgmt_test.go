...
func TestInstantiateCCDiscoveryError(t *testing.T) {
	...
	// Test InstantiateCC create discovery service error
	err := rc.InstantiateCC("error", req)
	if err == nil {
		t.Fatalf("Should have failed to instantiate cc with discovery error")
	}
	...
	// Test create discovery service error
	err = rc.InstantiateCCWithOpts("mychannel", req, resmgmt.InstantiateCCOpts{})
	if err == nil {
		t.Fatalf("Should have failed to instantiate cc with opts with discovery error")
	}
...
func TestCCProposal(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Failed to start mock event hub")
	}
	...
	err = rc.UpgradeCC("mychannel", upgradeRequest)
	if err == nil {
		t.Fatalf("Should have failed to error in commit")
	}

	// Test invalid function (only 'instatiate' and 'upgrade' are supported)
	err = rc.sendCCProposalWithOpts("invalid", "mychannel", instantiateReq, resmgmt.InstantiateCCOpts{Targets: peers})
	if err == nil {
