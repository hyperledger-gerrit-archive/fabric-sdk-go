...
func TestJoinChannelFail(t *testing.T) {
	...
	rc := setupResMgmtClient(ctx, nil, false, t)
...
func TestJoinChannelWithFilter(t *testing.T) {
	...
	rc := setupResMgmtClient(ctx, nil, true, t)
...
func TestJoinChannelDiscoveryError(t *testing.T) {
	...
	rc := setupResMgmtClient(ctx, errors.New("Test Error"), false, t)
...
func setupDefaultResMgmtClient(t *testing.T) *ResourceMgmtClient {
	...
	return setupResMgmtClient(ctx, nil, false, t)
}

func setupResMgmtClient(fabCtx fab.Context, discErr error, setTargetFilter bool, t *testing.T) *ResourceMgmtClient {
	...
	var resClient *ResourceMgmtClient

	if setTargetFilter {
		rcFilter := &MSPFilter{mspID: "Org1MSP"}
		resClient, err = New(ctx, WithTargetFilter(rcFilter))
		if err != nil {
			t.Fatalf("Failed to create new channel management client: %s", err)
		}
	} else {
		resClient, err = New(ctx)
		if err != nil {
			t.Fatalf("Failed to create new channel management client: %s", err)
		}
	}
