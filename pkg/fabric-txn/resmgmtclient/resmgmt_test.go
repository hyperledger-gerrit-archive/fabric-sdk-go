...
func TestNoSigningUserFailure(t *testing.T) {
	...
	ctx := Context{
		...
		Resource:          resource,
		ChannelService:    &mockChannelSvc{client},
...
func setupResMgmtClient(client *fcmocks.MockClient, discErr error, config apiconfig.Config, t *testing.T) *ResourceMgmtClient {
	...
	eventHub, err := getEventHub(client, client)
	if err != nil {
		t.Fatalf("Failed to setup event hub: %s", err)
	}
...
func getEventHub(pc fab.ProviderContext, ic fab.IdentityContext) (fab.EventHub, error) {
	ctx := events.Context{
		ProviderContext: pc,
		IdentityContext: ic,
	}
