...
func TestNoSigningUserFailure(t *testing.T) {
	...
	_, err = NewResourceMgmtClient(client, client.IdentityContext(), resource, &mockChannelSvc{client}, discovery, nil)
	if err == nil {
...
func setupResMgmtClient(client *fcmocks.MockClient, discErr error, config apiconfig.Config, t *testing.T) *ResourceMgmtClient {
	...
	resClient, err := NewResourceMgmtClient(client, client.IdentityContext(), resource, &mockChannelSvc{client}, discovery, nil)
	if err != nil {
