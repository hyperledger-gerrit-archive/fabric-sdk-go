...
func New(ctx Context, filter resmgmt.TargetFilter) (*ResourceMgmtClient, error) {
	...
	resourceClient := &ResourceMgmtClient{
		...
		channelSvc:        ctx.ChannelService,
		resource:          ctx.Resource,
		discovery:         discovery,
		eventHub:          ctx.EventHub,
		filter:            rcFilter,
	}
...
func (rc *ResourceMgmtClient) sendCCProposalWithOpts(ccProposalType CCProposalType, channelID string, req resmgmt.InstantiateCCRequest, opts resmgmt.InstantiateCCOpts) error {
	...
	if rc.eventHub.IsConnected() == false {
		err := rc.eventHub.Connect()
		if err != nil {
			return err
		}
		defer rc.eventHub.Disconnect()
	}

	// Register for commit event
	statusNotifier := internal.RegisterTxEvent(txID, rc.eventHub)
