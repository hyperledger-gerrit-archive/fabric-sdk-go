...
func (rc *ResourceMgmtClient) IsChaincodeInstalled(req resmgmt.InstallCCRequest, peer fab.Peer) (bool, error) {
	...
	for _, chaincode := range chaincodeQueryResponse.Chaincodes {
		if chaincode.Name == req.ID && chaincode.Version == req.Version && chaincode.Path == req.Path {
			return true, nil
		}
	}
...
func (rc *ResourceMgmtClient) InstallCCWithOpts(req resmgmt.InstallCCRequest, opts resmgmt.InstallCCOpts) ([]resmgmt.InstallCCResponse, error) {
	...
	transactionProposalResponse, _, err := rc.client.InstallChaincode(req.ID, req.Path, req.Version, req.DeployPath, opts.Package, peer.PeersToTxnProcessors(newTargets))
	if err != nil {
	...
	for _, v := range transactionProposalResponse {
		...
		logger.Infof("Install chaincode '%s' endorser '%s' returned ProposalResponse status:%v, error:'%s'", req.ID, v.Endorser, v.Status, v.Err)
...
func checkRequiredInstallCCParams(req resmgmt.InstallCCRequest) error {
	if req.ID == "" || req.Version == "" || req.Path == "" {
		return errors.New("Chaincode name(ID), version and path are required")
	}
	return nil
}
