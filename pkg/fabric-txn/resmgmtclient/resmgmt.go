...
// InstantiateCCWithOpts instantiates chaincode with custom options
func (rc *ResourceMgmtClient) sendCCProposalWithOpts(fn string, channelID string, req resmgmt.InstantiateCCRequest, opts resmgmt.InstantiateCCOpts) error {
...
func (rc *ResourceMgmtClient) getEventHub(channelID string) (*events.EventHub, error) {
	...
	var eventSources []*config.ChannelPeer
	for _, p := range peerConfig {
		if p.EventSource && p.MspID == rc.client.UserContext().MspID() {
			eventSources = append(eventSources, &p)
		}
	}
	...
	// In case of multiple event sources connect to random one
	rs := rand.NewSource(time.Now().Unix())
	r := rand.New(rs)
	choice := r.Intn(len(eventSources))

	eventSource := eventSources[choice]
