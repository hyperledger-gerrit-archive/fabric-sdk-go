...
type selectionService struct {
	...
	ccPolicyProvider fab.CCPolicyService
}

// NewSelectionService creates a selection service
func (p *SelectionProvider) NewSelectionService(ccPolicyProvider fab.CCPolicyService) (fab.SelectionService, error) {
	if ccPolicyProvider == nil {
		return nil, errors.New("Must provide chaincode policy service for dynamic selection")
	}
...
func (s *selectionService) GetEndorsersForChaincode(channelID string, channelPeers []fab.Peer,
	...
	resolver, err := s.getPeerGroupResolver(channelID, channelPeers, chaincodeIDs)
	if err != nil {
		return nil, errors.WithMessage(err, fmt.Sprintf("Error getting peer group resolver for chaincodes [%v] on channel [%s]", chaincodeIDs, channelID))
	}
	return resolver.Resolve().Peers(), nil
}

func (s *selectionService) getPeerGroupResolver(channelID string, channelPeers []fab.Peer, chaincodeIDs []string) (pgresolver.PeerGroupResolver, error) {
	...
	if resolver == nil {
		...
		if resolver, err = s.createPGResolver(channelPeers, key); err != nil {
			return nil, errors.WithMessage(err, fmt.Sprintf("unable to create new peer group resolver for chaincode(s) [%v] on channel [%s]", chaincodeIDs, channelID))
		}
	}
	return resolver, nil
}
...
func (s *selectionService) getPolicyGroupForCC(channelID string, ccID string, channelPeers []fab.Peer) (pgresolver.Group, error) {
	sigPolicyEnv, err := s.ccPolicyProvider.GetChaincodePolicy(channelID, ccID)
	if err != nil {
