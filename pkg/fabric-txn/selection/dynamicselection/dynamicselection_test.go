...
func verify(t *testing.T, service apifabclient.SelectionService, expectedPeerGroups []pgresolver.PeerGroup, channelID string, channelPeers []apifabclient.Peer, chaincodeIDs ...string) {
	...
	for i := 0; i < len(expectedPeerGroups); i++ {
		peers, err := service.GetEndorsersForChaincode(channelID, channelPeers, chaincodeIDs...)
		if err != nil {
...
func newMockSelectionService(ccPolicyProvider apifabclient.CCPolicyService, lbp pgresolver.LoadBalancePolicy) apifabclient.SelectionService {
...
func (p *mockCCDataProvider) GetChaincodePolicy(channelID string, chaincodeID string) (*common.SignaturePolicyEnvelope, error) {
	return unmarshalPolicy(p.ccData[newResolverKey(channelID, chaincodeID).String()].Policy)
}
