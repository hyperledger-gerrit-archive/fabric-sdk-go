...
func TestDiscoveryBlocklist(t *testing.T) {
	testPeer1 := fcmocks.NewMockPeer("Peer1", "http://peer1.com")
	...
	assert.True(t, ok, "expected status error")
	assert.EqualValues(t, status.NoPeersFound.ToInt32(), s.Code, "expected No Peers Found status on blocklist")
	assert.Equal(t, 1, testPeer1.ProcessProposalCalls, "expected peer 1 to be blocklisted")
	// Wait for blocklist expiry
	time.Sleep(ctx.Config().TimeoutOrDefault(apiconfig.DiscoveryBlocklistExpiry))
	...
	assert.Equal(t, attempts+1, testPeer1.ProcessProposalCalls, "expected peer 1 not to be blocklisted")
}
