...
func TestQueryWithOptAsync(t *testing.T) {
	...
	queryOpts := []apitxn.Option{WithNotifier(notifier)}

	result, err := chClient.Query(apitxn.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}}, queryOpts...)
	if err != nil {
		t.Fatalf("Failed to invoke test cc: %s", err)
	}
...
func TestQueryWithOptTarget(t *testing.T) {
	...
	queryOpts := []apitxn.Option{WithProposalProcessor(targets)}
...
func TestExecuteTx(t *testing.T) {
	...
	_, _, err := chClient.ExecuteTx(apitxn.Request{})
	if err == nil {
		t.Fatalf("Should have failed for empty invoke request")
	}

	_, _, err = chClient.ExecuteTx(apitxn.Request{Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	if err == nil {
		t.Fatalf("Should have failed for empty chaincode ID")
	}

	_, _, err = chClient.ExecuteTx(apitxn.Request{ChaincodeID: "testCC", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	if err == nil {
		t.Fatalf("Should have failed for empty function")
	}
...
func TestExecuteTxDiscoveryError(t *testing.T) {
	...
	_, _, err := chClient.ExecuteTx(apitxn.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	if err == nil {
...
func TestOrdererStatusError(t *testing.T) {
	...
	_, _, err := chClient.ExecuteTx(apitxn.Request{ChaincodeID: "test", Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	statusError, ok := status.FromError(err)
...
func TestTransactionValidationError(t *testing.T) {
	...
	notifier := make(chan apitxn.Response)
	exOpts := []apitxn.Option{WithNotifier(notifier)}
	result, _, err := chClient.ExecuteTx(apitxn.Request{ChaincodeID: "test", Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}},
		exOpts...)
	if result != nil {
		t.Fatalf("Expecting nil, got %s", result)
	}
