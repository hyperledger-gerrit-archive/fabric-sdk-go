...
import (
	...
	"github.com/hyperledger/fabric-sdk-go/pkg/fabric-txn/discovery/blocklist"
...
func New(c Context) (*ChannelClient, error) {
	blocklistProvider := blocklist.New(c.Config().TimeoutOrDefault(apiconfig.DiscoveryBlocklistExpiry))

	channelClient := ChannelClient{
		blocklist: blocklistProvider,
		context:   c,
		discovery: discovery.NewDiscoveryFilterService(c.DiscoveryService, blocklistProvider),
...
func (cc *ChannelClient) resolveRetry(req *chclient.RequestContext, opts chclient.Opts) bool {
	...
	cc.blocklist.Blocklist(req.Error)
	// Reset processors
