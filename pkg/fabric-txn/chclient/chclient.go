...
func (cc *ChannelClient) QueryWithOpts(request apitxn.QueryRequest, opts apitxn.QueryOpts) (string, error) {
	...
	case <-time.After(time.Second * 20):
		return "", fmt.Errorf("Query request timed out")
	}
...
func sendTransactionProposal(request apitxn.QueryRequest, channel fab.Channel, proposalProcessors []apitxn.ProposalProcessor, notifier chan apitxn.QueryResponse) {
	...
	// TODO: Should we change response to bytes too ???
...
func (cc *ChannelClient) ExecuteTxWithOpts(request apitxn.ExecuteTxRequest, opts apitxn.ExecuteTxOpts) (apitxn.TransactionID, error) {
	...
	go sendTransaction(cc.channel, txID, txProposalResponses, cc.eventHub, notifier)
...
func sendTransaction(channel fab.Channel, txID apitxn.TransactionID, txProposalResponses []*apitxn.TransactionProposalResponse, eventHub fab.EventHub, notifier chan apitxn.ExecuteTxResponse) {
