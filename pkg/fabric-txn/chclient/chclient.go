...
import (
	...
	"github.com/hyperledger/fabric-sdk-go/pkg/fabric-txn/discovery/blacklist"
...
func New(c Context) (*ChannelClient, error) {
	blacklistProvider := blacklist.New(c.Config().TimeoutOrDefault(apiconfig.DiscoveryBlacklistExpiry))

	channelClient := ChannelClient{
		blacklist: blacklistProvider,
		context:   c,
		discovery: discovery.NewDiscoveryFilterService(c.DiscoveryService, blacklistProvider),
...
func (cc *ChannelClient) resolveRetry(req *chclient.RequestContext, opts chclient.Opts) bool {
	...
	cc.blacklist.Blacklist(req.Error)
	// Reset processors
