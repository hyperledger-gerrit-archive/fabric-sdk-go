...
const (
	queryTimeout     = time.Second * 20
	executeTxTimeout = time.Second * 30
)
...
func (cc *ChannelClient) Query(request apitxn.QueryRequest) ([]byte, error) {
	...
	return cc.QueryWithOpts(request, apitxn.QueryOpts{Timeout: queryTimeout})
...
func (cc *ChannelClient) ExecuteTx(request apitxn.ExecuteTxRequest) (apitxn.TransactionID, error) {
	...
	return cc.ExecuteTxWithOpts(request, apitxn.ExecuteTxOpts{Timeout: executeTxTimeout})
}
