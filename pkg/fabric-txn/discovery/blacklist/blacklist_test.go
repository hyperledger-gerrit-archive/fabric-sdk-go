...
func TestBlackListFilter(t *testing.T) {
	...
	for index, badPeer := range badPeers {
		f.Blacklist(connectionFailedStatus(badPeer.URL()))
		assert.False(t, f.Accept(badPeer), "Expected bad peer to be blacklisted")
...
func TestBlackListInvalidErr(t *testing.T) {
	f := New(time.Microsecond * 1)
	f.Blacklist(fmt.Errorf("test"))
...
func createMockPeers(fromIndex int, toIndex int) []apifabclient.Peer {
	var mockPeers []apifabclient.Peer
