...
func TestBlockListFilter(t *testing.T) {
	...
	for index, badPeer := range badPeers {
		f.Blocklist(connectionFailedStatus(badPeer.URL()))
		assert.False(t, f.Accept(badPeer), "Expected bad peer to be blocklisted")
...
func TestBlockListInvalidErr(t *testing.T) {
	f := New(time.Microsecond * 1)
	f.Blocklist(fmt.Errorf("test"))
