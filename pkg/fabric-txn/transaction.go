...
import (
	...
	event "github.com/hyperledger/fabric-sdk-go/api/apievent"
...
func InvokeChaincode(client fab.FabricClient, channel fab.Channel, targets []apitxn.ProposalProcessor,
	eventHub event.EventHub, chaincodeID string, args []string, transientData map[string][]byte) error {
...
func RegisterCCEvent(chainCodeID string, eventID string, eventHub event.EventHub) (chan bool, *event.ChainCodeCBE) {
	done := make(chan bool)

	// Register callback for CE
	rce := eventHub.RegisterChaincodeEvent(chainCodeID, eventID, func(ce *event.ChaincodeEvent) {
