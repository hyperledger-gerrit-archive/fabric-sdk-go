...
func QueryChaincode(client api.FabricClient, channel api.Channel, chaincodeID string, args []string) (string, error) {
	...
	transactionProposalResponses, _, err := internal.CreateAndSendTransactionProposal(channel,
		chaincodeID, channel.Name(), args, []txnapi.TxnProposalProcessor{channel.PrimaryPeer()}, nil)
...
// InvokeChaincode ...
func InvokeChaincode(client api.FabricClient, channel api.Channel, targets []txnapi.TxnProposalProcessor,
