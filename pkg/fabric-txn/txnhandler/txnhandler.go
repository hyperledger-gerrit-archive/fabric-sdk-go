...
func (e *EndorseTxHandler) Handle(requestContext *txnhandler.RequestContext, clientContext *txnhandler.ClientContext) {
	...
	if len(txProcessors) == 0 {
		...
		if err != nil {
			requestContext.ResponseCollector <- apitxn.Response{Payload: nil, Error: errors.WithMessage(err, "GetPeers failed")}
			return
		}
		endorsers := peers
		if clientContext.Selection != nil {
			...
			if err != nil {
				requestContext.ResponseCollector <- apitxn.Response{Payload: nil, Error: errors.WithMessage(err, "Failed to get endorsing peers")}
				return
			}
		}
	...
	if err != nil {
		requestContext.ResponseCollector <- apitxn.Response{Payload: nil, TransactionID: txnID, Error: err}
		return
	}
	...
	if e.next != nil {
		...
		requestContext.ResponseCollector <- apitxn.Response{Payload: response, TransactionID: txnID, Responses: transactionProposalResponses}
	}
}
...
func (f *EndorsementValidationHandler) Handle(requestContext *txnhandler.RequestContext, clientContext *txnhandler.ClientContext) {
	...
	if err != nil {
		requestContext.ResponseCollector <- apitxn.Response{Payload: nil, TransactionID: requestContext.Response.TransactionID, Error: errors.WithMessage(err, "TxFilter failed")}
		return
	}
	...
	if f.next != nil {
		...
		requestContext.ResponseCollector <- apitxn.Response{Payload: response, Error: nil}
	}
}
...
func (c *CommitTxHandler) Handle(requestContext *txnhandler.RequestContext, clientContext *txnhandler.ClientContext) {
	...
	if clientContext.EventHub.IsConnected() == false {
		...
		if err != nil {
			requestContext.ResponseCollector <- apitxn.Response{TransactionID: apitxn.TransactionID{}, Error: err}
		}
	}
	...
	if err != nil {
		requestContext.ResponseCollector <- apitxn.Response{TransactionID: apitxn.TransactionID{}, Error: errors.Wrap(err, "CreateAndSendTransaction failed")}
		return
	}
	...
	case result := <-statusNotifier:
		if result.Error == nil {
			requestContext.ResponseCollector <- apitxn.Response{Payload: requestContext.Response.Payload, TransactionID: txnID, TxValidationCode: result.Code}
		} else {
			requestContext.ResponseCollector <- apitxn.Response{Payload: requestContext.Response.Payload, TransactionID: txnID, TxValidationCode: result.Code, Error: result.Error}
		}
	case <-time.After(requestContext.Opts.Timeout):
		requestContext.ResponseCollector <- apitxn.Response{TransactionID: txnID, Error: errors.New("Execute didn't receive block event")}
	}
