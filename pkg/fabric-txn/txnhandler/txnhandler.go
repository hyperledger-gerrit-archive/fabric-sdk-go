...
func (c *CommitTxHandler) Handle(requestContext *txnhandler.RequestContext, clientContext *txnhandler.ClientContext) {
	...
	case <-time.After(requestContext.Opts.Timeout):
		requestContext.Opts.Notifier <- apitxn.Response{TransactionID: txnID, Error: errors.New("ExecuteTx didn't receive block event")}
	}
...
//GetQueryHandler returns query handler with EndorseTxHandler & EndorsementValidationHandler Chained
func GetQueryHandler() txnhandler.Handler {
	return &EndorseTxHandler{&EndorsementValidationHandler{}}
}

//GetExecuteTxHandler returns query handler with EndorseTxHandler, EndorsementValidationHandler & CommitTxHandler Chained
