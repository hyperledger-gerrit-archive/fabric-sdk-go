...
func (f *FilterTxHandler) Handle(requestContext *txnhandler.RequestContext, clientContext *txnhandler.ClientContext) {
	...
	var err error
	requestContext.Response.Responses, err = f.processTxProposalResponse(requestContext.Response.Responses)
	if err != nil {
		requestContext.Request.Opts.Notifier <- apitxn.Response{Payload: nil, TransactionID: requestContext.Response.TransactionID, Error: errors.WithMessage(err, "TxFilter failed")}
		return
	}
...
func (f *FilterTxHandler) processTxProposalResponse(txProposalResponse []*apitxn.TransactionProposalResponse) ([]*apitxn.TransactionProposalResponse, error) {
	var a1 []byte
