 ...
 */
func NewMockUser(name string) identity.User {
	return &MockUser{name: name}
}

//NewMockUserWithMSPID to return mock user with MSP ids
func NewMockUserWithMSPID(name string, mspid string) identity.User {
...
// IsEnrolled returns true if the enrollment cert is present
