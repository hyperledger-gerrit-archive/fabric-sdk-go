...
// PackageGoLangCC ...
func PackageGoLangCC(chaincodePath string, deployPath string) ([]byte, error) {
	...
	if deployPath == "" {
		...
		projDir = path.Join(deployPath, "src", chaincodePath)
	}
	...
	descriptors, err := findSource(deployPath, projDir)
	if err != nil {
		return nil, err
	}
