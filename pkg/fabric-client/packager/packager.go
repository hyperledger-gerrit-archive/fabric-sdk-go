 ...
 * @param {string} goPath optional - override GOPATH. If empty default GOPATH will be used.
 * @returns {[]byte} byte array
 */
func PackageCC(chaincodePath string, chaincodeType string, goPath string) ([]byte, error) {
	...
	case "golang":
		return PackageGoLangCC(chaincodePath, goPath)
	}
