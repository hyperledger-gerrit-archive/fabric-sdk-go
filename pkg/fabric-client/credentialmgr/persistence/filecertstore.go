...
// NewFileCertStore ...
func NewFileCertStore(cryptoConfogMspPath string) (apifabclient.KeyValueStore, error) {
	...
	opts := &keyvaluestore.FileKeyValueStoreOptions{
		...
		KeySerializer: func(storePath string, key interface{}) (string, error) {
			...
			certDir := path.Join(strings.Replace(storePath, "{userName}", ck.UserName, -1), "signcerts")
