...
func getCertStore(config apiconfig.Config, orgName string) (kvstore.KVStore, error) {
	...
	//return nil, fmt.Errorf("getCertStore: orgCryptoPathTemplate: %s\n", orgCryptoPathTemplate)
	// if orgCryptoPathTemplate == "" {
	// 	return nil, errors.Errorf("getCertStore: Cryptopath not provided for organization [%s], store(s) not created", orgName)
	// }
