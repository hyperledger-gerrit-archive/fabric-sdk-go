...
func NewEventsClient(provider context.ProviderContext, identity context.IdentityContext, peerAddress string, certificate *x509.Certificate,
	...
	kap keepalive.ClientParameters, failFast bool, allowInsecure bool) (context.EventsClient, error) {
...
func newEventsClientConnectionWithAddress(peerAddress string, cert *x509.Certificate, serverHostOverride string,
	...
	opts = append(opts, grpc.WithTimeout(config.TimeoutOrDefault(apiconfig.EventHubConnection)))
	if secured {
	...
	ctx, _ = grpcContext.WithTimeout(ctx, config.TimeoutOrDefault(apiconfig.EventHubConnection))
...
func (ec *eventsClient) Stop() error {
	...
	case <-time.After(ec.provider.Config().TimeoutOrDefault(apiconfig.EventHubConnection)):
