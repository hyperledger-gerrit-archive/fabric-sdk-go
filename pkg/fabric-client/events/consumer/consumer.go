...
func (ec *eventsClient) establishConnectionAndRegister(secured bool) error {
	...
	if err != nil {
		...
		if secured && ec.allowInSecure {
			//If secured mode failed and allow in-secure is enabled then retry in insecure mode
			logger.Debug("Secured establishConnectionAndRegister failed, attempting in-secured")
