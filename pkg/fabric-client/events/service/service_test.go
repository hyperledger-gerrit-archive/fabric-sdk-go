...
func TestConcurrentEvents(t *testing.T) {
	...
	opts := DefaultOpts()
	opts.EvtConsumerBufferSize = numEvents * 4

	eventService, eventProducer, err := newServiceWithMockProducer(opts, withBlockLedger())
	if err != nil {
		t.Fatalf("error creating channel event client: %s", err)
	}
...
func newServiceWithMockProducer(opts Options, pOpts ...producerOpt) (*Service, *servicemocks.MockProducer, error) {
