...
// Right now Service doesn't define its own options
...
func New(dispatcher Dispatcher, opts Options) *Service {
	return &Service{
		opts:       opts,
		dispatcher: dispatcher,
	}
...
func (s *Service) Stop() {
	...
	eventch <- dispatcher.NewStopEvent(respch)
...
func (s *Service) RegisterBlockEvent(filter ...apifabclient.BlockFilter) (apifabclient.Registration, <-chan *apifabclient.BlockEvent, error) {
	eventch := make(chan *apifabclient.BlockEvent, s.opts.EventConsumerBufferSize())
	...
	if err := s.Submit(dispatcher.NewRegisterBlockEvent(blockFilter, eventch, respch)); err != nil {
...
func (s *Service) RegisterFilteredBlockEvent() (apifabclient.Registration, <-chan *apifabclient.FilteredBlockEvent, error) {
	eventch := make(chan *apifabclient.FilteredBlockEvent, s.opts.EventConsumerBufferSize())
	respch := make(chan *apifabclient.RegistrationResponse)
	if err := s.Submit(dispatcher.NewRegisterFilteredBlockEvent(eventch, respch)); err != nil {
...
func (s *Service) RegisterChaincodeEvent(ccID, eventFilter string) (apifabclient.Registration, <-chan *apifabclient.CCEvent, error) {
	...
	eventch := make(chan *apifabclient.CCEvent, s.opts.EventConsumerBufferSize())
	respch := make(chan *apifabclient.RegistrationResponse)
	if err := s.Submit(dispatcher.NewRegisterChaincodeEvent(ccID, eventFilter, eventch, respch)); err != nil {
...
func (s *Service) RegisterTxStatusEvent(txID string) (apifabclient.Registration, <-chan *apifabclient.TxStatusEvent, error) {
	...
	eventch := make(chan *apifabclient.TxStatusEvent, s.opts.EventConsumerBufferSize())
	respch := make(chan *apifabclient.RegistrationResponse)
	if err := s.Submit(dispatcher.NewRegisterTxStatusEvent(txID, eventch, respch)); err != nil {
