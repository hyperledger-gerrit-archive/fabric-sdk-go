...
func (c *MockProducer) listen() {
	for {
		...
		fmt.Printf("MockProducer.listen - notifying all consumers\n")
		c.notifyAll(event)
	}
}

func (c *MockProducer) notifyAll(event interface{}) {
	fmt.Printf("MockProducer.notifyAll\n")
	c.RLock()
	defer c.RUnlock()

	fmt.Printf("MockProducer.notifyAll - Looping...\n")
	for _, eventch := range c.eventChannels {
		fmt.Printf("MockProducer.notifyAll - Notifying event channel...\n")
		send(eventch, event)
		fmt.Printf("MockProducer.notifyAll - Notified event channel.\n")
	}
}
...
func send(eventch chan<- interface{}, event interface{}) {
	defer func() {
		...
		if p := recover(); p != nil {
			fmt.Printf("panic while submitting event %#v: %s", event, p)
		}
	}()

	fmt.Printf("MockProducer.send - Sending to event channel...\n")
