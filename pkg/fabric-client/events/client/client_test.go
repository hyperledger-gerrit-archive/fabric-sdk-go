...
import (
	...
	mockconn "github.com/hyperledger/fabric-sdk-go/pkg/fabric-client/events/client/mocks"
...
func TestConnect(t *testing.T) {
	...
	eventClient, _, err := newClientWithMockConnAndOpts("mychannel", newMockContext(), connectionProvider, DefaultOpts(), filteredClientProvider, discoveryService)
	if err != nil {
...
func TestFailConnect(t *testing.T) {
	eventClient, _, err := newClientWithMockConnAndOpts(
		...
		DefaultOpts(), failAfterConnectClientProvider,
...
func TestConcurrentEvents(t *testing.T) {
	...
	opts := DefaultOpts()
	opts.EvtConsumerBufferSize = uint(numEvents) * 4

	eventClient, conn, err := newClientWithMockConnAndOpts(
		channelID, newMockContext(),
		nil, opts, clientProvider,
		clientmocks.NewDiscoveryService(peer1, peer2),
		mockconn.WithLedger(servicemocks.NewMockLedger(servicemocks.BlockEventFactory)),
	)
	if err != nil {
...
func listenBlockEvents(channelID string, eventch <-chan *apifabclient.BlockEvent, expected int, errch chan<- error) {
	numReceived := 0
...
func listenFilteredBlockEvents(channelID string, eventch <-chan *apifabclient.FilteredBlockEvent, expected int, errch chan<- error) {
	numReceived := 0
...
func listenChaincodeEvents(channelID string, eventch <-chan *apifabclient.CCEvent, expected int, errch chan<- error) {
	numReceived := 0
...
func txStatusTest(eventClient Client, ledger servicemocks.Ledger, channelID string, expected int, errch chan<- error) {
	...
	if len(errs) > 0 {
		errch <- errors.Errorf("Received %d events and %d errors. First error %s\n", receivedEvents, len(errs), errs[0])
	} else {
		errch <- nil
	}
}
...
func testConnect(t *testing.T, maxConnectAttempts uint, expectedOutcome mockconn.Outcome, connAttemptResult mockconn.ConnectAttemptResults) {
	...
	opts := DefaultOpts()
	opts.EvtConsumerTimeout = 1 * time.Second
	opts.MaxConnAttempts = maxConnectAttempts
...
func testReconnect(t *testing.T, reconnect bool, maxReconnectAttempts uint, expectedOutcome mockconn.Outcome, connAttemptResult mockconn.ConnectAttemptResults) {
	...
	opts := DefaultOpts()
	opts.ReconnInitialDelay = 0
	opts.EvtConsumerTimeout = 3 * time.Second
	opts.Reconn = reconnect
	opts.MaxConnAttempts = 1
	opts.MaxReconnAttempts = maxReconnectAttempts
	opts.TimeBetweenConnAttempts = time.Millisecond
	opts.ConnEventCh = connectch
...
func listenConnection(eventch chan *apifabclient.ConnectionEvent, outcome chan mockconn.Outcome) {
	state := InitialState
...
func listenEvents(blockch <-chan *apifabclient.BlockEvent, ccch <-chan *apifabclient.CCEvent, waitDuration time.Duration, numEventsCh chan mockconn.Received, expectedBlockEvents mockconn.NumBlock, expectedCCEvents mockconn.NumChaincode) {
...
type ClientProvider func(channelID string, context apifabclient.Context, connectionProvider connection.Provider, discoveryService apifabclient.DiscoveryService, opts Options) (Client, error)

var clientProvider = func(channelID string, context apifabclient.Context, connectionProvider connection.Provider, discoveryService apifabclient.DiscoveryService, opts Options) (Client, error) {
...
var failAfterConnectClientProvider = func(channelID string, context apifabclient.Context, connectionProvider connection.Provider, discoveryService apifabclient.DiscoveryService, opts Options) (Client, error) {
	return newClient(channelID, context, connectionProvider, discoveryService, opts, true,
		func() error {
...
var filteredClientProvider = func(channelID string, context apifabclient.Context, connectionProvider connection.Provider, discoveryService apifabclient.DiscoveryService, opts Options) (Client, error) {
...
func newClient(channelID string, context apifabclient.Context, connectionProvider connection.Provider, discoveryService apifabclient.DiscoveryService, opts Options, permitBlockEvents bool, afterConnect handler, beforeReconnect handler) (Client, error) {
	client := New(
		opts, permitBlockEvents,
		dispatcher.New(
			channelID, context,
			connectionProvider,
			discoveryService,
...
func newClientWithMockConn(channelID string, context apifabclient.Context, clientProvider ClientProvider, discoveryService apifabclient.DiscoveryService, connOpts ...mockconn.Opt) (Client, mockconn.Connection, error) {
	conn := mockconn.NewMockConnection(connOpts...)
	client, _, err := newClientWithMockConnAndOpts(channelID, context, mockconn.NewProviderFactory().Provider(conn), DefaultOpts(), clientProvider, discoveryService)
	return client, conn, err
}

func newClientWithMockConnAndOpts(channelID string, context apifabclient.Context, connectionProvider connection.Provider, opts Options, clientProvider ClientProvider, discoveryService apifabclient.DiscoveryService, connOpts ...mockconn.Opt) (Client, mockconn.Connection, error) {
