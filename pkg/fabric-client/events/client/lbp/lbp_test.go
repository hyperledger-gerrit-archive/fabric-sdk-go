...
var (
	p1 = fabclientmocks.NewMockPeer("p1", "")
	p2 = fabclientmocks.NewMockPeer("p2", "")
	p3 = fabclientmocks.NewMockPeer("p3", "")
	p4 = fabclientmocks.NewMockPeer("p4", "")
)
...
func TestRandom(t *testing.T) {
	...
	peer, err := lbp.Choose([]apifabclient.Peer{})
	if err != nil {
	...
	numInvocations := 10
	var lastPeerChosen apifabclient.Peer
...
func findIndex(peers []apifabclient.Peer, peer apifabclient.Peer) int {
	for i, p := range peers {
		if peer == p {
			return i
		}
...
func newMockPeers(numPeers int) []apifabclient.Peer {
	var peers []apifabclient.Peer
	for i := 0; i < numPeers; i++ {
		peers = append(peers, fabclientmocks.NewMockPeer(fmt.Sprintf("peer_%d", i), ""))
	}
	return peers
}
