...
func TestConnection(t *testing.T) {
	...
	conn, err = New(channelID, context, "invalidhost:0000", "", testStream)
	if err == nil {
		t.Fatalf("expected error creating new connection with invalid URL")
	}
	conn, err = New(channelID, context, peerURL, "", invalidStream)
	if err == nil {
		t.Fatalf("expected error creating new connection with invalid stream but got none")
	}

	conn, err = New(channelID, context, peerURL, "", testStream)
	if err != nil {
