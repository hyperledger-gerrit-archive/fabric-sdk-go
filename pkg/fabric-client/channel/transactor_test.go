...
func createTransactionProposal(t *testing.T, transactor apifabclient.Transactor) *apifabclient.TransactionProposal {
	...
	tp, err := transactor.CreateTransactionProposal(request)
	assert.Nil(t, err)
...
// TestOrderersFromChannelCfg uses an orderer that does not exist in the configuration and doesn't
// exist in the TLS cert pool.
...
/*
// TODO: needs adjustments to integration tests to enable.
func TestNoOrderersConfigured(t *testing.T) {
	user := mocks.NewMockUser("test")
	ctx := mocks.NewMockContext(user)
	chConfig := mocks.NewMockChannelCfg("testChannel")
	chConfig.(*mocks.MockChannelCfg).MockOrderers = []string{"doesnotexist.com"}

	_, err := NewTransactor(ctx, chConfig)
