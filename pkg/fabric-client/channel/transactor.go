...
func orderersFromChannelCfg(ctx fab.Context, cfg fab.ChannelCfg) ([]fab.Orderer, error) {
	...
	for _, target := range cfg.Orderers() {
		...
		if !ok {
			...
			o, err := orderer.New(ctx.Config(), orderer.WithURL("grpcs://"+target))
...
func (t *Transactor) CreateChaincodeInvokeProposal(request fab.ChaincodeInvokeProposal) (*fab.TransactionProposal, error) {
