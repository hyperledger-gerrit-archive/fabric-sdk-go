...
// channelChaincodeInvokeRequest contains the parameters for a chaincode invocation on a channel
type channelChaincodeInvokeRequest struct {
	apitxn.ChaincodeInvokeRequest
	ChannelID string
}
...
func (c *Channel) SendTransactionProposal(request apitxn.ChaincodeInvokeRequest) ([]*apitxn.TransactionProposalResponse, apitxn.TransactionID, error) {
	request, err := updateChaincodeInvokeRequest(request, c.clientContext)
	requestWithChannel := channelChaincodeInvokeRequest{
		ChaincodeInvokeRequest: request,
		ChannelID:              c.name,
	}

	proposal, err := newTransactionProposal(requestWithChannel, c.clientContext)
	if err != nil {
		return nil, request.TxnID, err
	}

	responses, err := c.sendTransactionProposalPB(proposal, request.Targets)
	return responses, request.TxnID, err
}

func updateChaincodeInvokeRequest(request apitxn.ChaincodeInvokeRequest, clientContext ClientContext) (apitxn.ChaincodeInvokeRequest, error) {
...
func sendTransactionProposal(channelID string, request apitxn.ChaincodeInvokeRequest, clientContext ClientContext) ([]*apitxn.TransactionProposalResponse, apitxn.TransactionID, error) {
	request, err := updateChaincodeInvokeRequest(request, clientContext)
	if err != nil {
		return nil, request.TxnID, err
	}
	...
	proposal, err := newTransactionProposal(requestWithChannel, clientContext)
	if err != nil {
		return nil, request.TxnID, err
	}

	responses, err := SendTransactionProposalPB(proposal, request.Targets)
	return responses, request.TxnID, err
}
...
// ECert to sign.
func newTransactionProposal(request channelChaincodeInvokeRequest, clientContext ClientContext) (*apitxn.TransactionProposal, error) {
	...
	proposal, _, err := protos_utils.CreateChaincodeProposalWithTxIDNonceAndTransient(request.TxnID.ID, common.HeaderType_ENDORSER_TRANSACTION, request.ChannelID, ccis, request.TxnID.Nonce, creator, request.TransientMap)
	if err != nil {
...
// sendTransactionProposalPB sends the created proposal to peer for endorsement.
func (c *Channel) sendTransactionProposalPB(proposal *apitxn.TransactionProposal, targets []apitxn.ProposalProcessor) ([]*apitxn.TransactionProposalResponse, error) {
	...
	if targets == nil || len(targets) == 0 {
		if c.peers == nil || len(c.peers) == 0 {
			return nil, fmt.Errorf("targets were not specified and no peers have been configured")
		}

		targets = c.txnProcessors()
	}

	return SendTransactionProposalPB(proposal, targets)
}

// SendTransactionProposalPB ... TODO (should be refactored)
func SendTransactionProposalPB(proposal *apitxn.TransactionProposal, targets []apitxn.ProposalProcessor) ([]*apitxn.TransactionProposalResponse, error) {
...
func (c *Channel) JoinChannel(request *fab.JoinChannelRequest) error {
	...
	// Send join proposal
	proposalResponses, err := SendTransactionProposalPB(transactionProposal, targets)
	if err != nil {
