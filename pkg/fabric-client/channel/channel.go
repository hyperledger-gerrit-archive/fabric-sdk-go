...
import (
	...
	config "github.com/hyperledger/fabric-sdk-go/pkg/config"
 ...
 */
func NewChannel(name string, client api.Client) (api.Channel, error) {
	if name == "" {
	...
	c := &channel{name: name, securityEnabled: config.IsSecurityEnabled(), peers: p,
...
func (c *channel) GetClientContext() api.Client {
 ...
 * ECert to sign.
 */
func (c *channel) CreateTransactionProposal(chaincodeName string, chainID string,
 ...
 */
func (c *channel) SendInstantiateProposal(chaincodeName string, chainID string,
