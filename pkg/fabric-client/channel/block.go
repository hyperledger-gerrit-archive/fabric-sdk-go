...
func (c *Channel) GenesisBlock() (*common.Block, error) {
	...
	seekInfoHeader, err := txn.CreateChannelHeader(common.HeaderType_DELIVER_SEEK_INFO, c.Name(), txnID.ID, 0, "", time.Now(), tlsCertHash)
	if err != nil {
		return nil, errors.Wrap(err, "BuildChannelHeader failed")
	}
	seekHeader, err := txn.CreateHeader(creator, seekInfoHeader, txnID.Nonce)
	if err != nil {
...
func (c *Channel) block(pos *ab.SeekPosition) (*common.Block, error) {
	...
	tlsCertHash := ccomm.TLSCertHash(c.clientContext.Config())
	seekInfoHeader, err := txn.CreateChannelHeader(common.HeaderType_DELIVER_SEEK_INFO, c.Name(), txID.ID, 0, "", time.Now(), tlsCertHash)
	if err != nil {
