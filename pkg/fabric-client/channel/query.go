...
func (c *Channel) QueryInfo() (*common.BlockchainInfo, error) {
	...
	payload, err := QueryBySystemChaincodeByTarget(NewChaincodeInvokeRequestForTarget("qscc", "GetChainInfo", args, c.PrimaryPeer()), c.clientContext)
	if err != nil {
...
func (c *Channel) QueryBlockByHash(blockHash []byte) (*common.Block, error) {
	...
	payload, err := QueryBySystemChaincodeByTarget(NewChaincodeInvokeRequestForTarget("qscc", "GetBlockByHash", args, c.PrimaryPeer()), c.clientContext)
	if err != nil {
...
func (c *Channel) QueryBlock(blockNumber int) (*common.Block, error) {
	...
	payload, err := QueryBySystemChaincodeByTarget(NewChaincodeInvokeRequestForTarget("qscc", "GetBlockByNumber", args, c.PrimaryPeer()), c.clientContext)
	if err != nil {
...
func (c *Channel) QueryTransaction(transactionID string) (*pb.ProcessedTransaction, error) {
	...
	payload, err := QueryBySystemChaincodeByTarget(NewChaincodeInvokeRequestForTarget("qscc", "GetTransactionByID", args, c.PrimaryPeer()), c.clientContext)
	if err != nil {
		return nil, fmt.Errorf("Invoke qscc GetBlockByNumber return error: %v", err)
	}
...
func (c *Channel) QueryInstantiatedChaincodes() (*pb.ChaincodeQueryResponse, error) {
	...
	payload, err := QueryBySystemChaincodeByTarget(NewChaincodeInvokeRequestForTarget("lscc", "getchaincodes", args, c.PrimaryPeer()), c.clientContext)
	if err != nil {
...
func (c *Channel) QueryByChaincode(request txn.ChaincodeInvokeRequest) ([][]byte, error) {
	if err := validateRequestForQuery(request); err != nil {
		return nil, err
	}

	transactionProposalResponses, _, err := c.SendTransactionProposal(request)
	if err != nil {
...
func validateRequestForQuery(request txn.ChaincodeInvokeRequest) error {
	...
	//if request.Args == nil {
	//	return fmt.Errorf("Missing invoke arguments")
	//}
