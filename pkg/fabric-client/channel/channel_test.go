...
func TestCreateTransaction(t *testing.T) {
	...
	res, err := channel.CreateTransaction([]*api.TransactionProposalResponse{})
	...
	test := &api.TransactionProposalResponse{
	...
	input := []*api.TransactionProposalResponse{test}

	res, err = channel.CreateTransaction(input)
	...
	res, err = channel.CreateTransaction([]*api.TransactionProposalResponse{test})
...
func TestBroadcastEnvelope(t *testing.T) {
	...
	if err == nil || err.Error() != "orderers not set"{
		t.Fatal("orderers not set validation on broadcat envelope is not working as expected")
	}
...
func TestSendTransactionProposal(t *testing.T) {
	...
	result, err = SendTransactionProposal(&api.TransactionProposal{
		SignedProposal: &pb.SignedProposal{},
	}, 1, []api.Peer{})
