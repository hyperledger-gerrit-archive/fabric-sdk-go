...
func (c *Resource) createOrUpdateChannel(request fab.CreateChannelRequest, haveEnvelope bool) error {
	...
	if haveEnvelope {
		...
		channelHeader, err := txn.CreateChannelHeader(common.HeaderType_CONFIG_UPDATE, request.Name, request.TxnID.ID, 0, "", time.Now(), tlsCertHash)
		if err != nil {
		...
		header, err := txn.CreateHeader(creator, channelHeader, request.TxnID.Nonce)
		if err != nil {
