...
func (c *Resource) CreateChannel(request context.CreateChannelRequest) (context.TransactionID, error) {
	if request.Orderer == nil {
		return context.TransactionID{}, errors.New("missing orderer request parameter for the initialize channel")
	}

	if request.Name == "" {
		return context.TransactionID{}, errors.New("missing name request parameter for the new channel")
	}
	...
	if request.Config == nil {
		return context.TransactionID{}, errors.New("missing envelope request parameter containing the configuration of the new channel")
	}

	if request.Signatures == nil {
		return context.TransactionID{}, errors.New("missing signatures request parameter for the new channel")
	}
...
func (c *Resource) createChannelFromEnvelope(request context.CreateChannelRequest) (context.TransactionID, error) {
	env, err := c.extractSignedEnvelope(request.Envelope)
	if err != nil {
		return context.TransactionID{}, errors.WithMessage(err, "signed envelope not valid")
	}

	// Send request
	_, err = request.Orderer.SendBroadcast(env)
	if err != nil {
		return context.TransactionID{}, errors.WithMessage(err, "failed broadcast to orderer")
	}
	return context.TransactionID{}, nil
}

// GenesisBlockFromOrderer returns the genesis block from the defined orderer that may be
// used in a join request
func (c *Resource) GenesisBlockFromOrderer(channelName string, orderer context.Orderer) (*common.Block, error) {

	orderers := []context.Orderer{orderer}
...
func (c *Resource) JoinChannel(request context.JoinChannelRequest) error {
	...
	pr := context.ChaincodeInvokeRequest{
		ChaincodeID: "cscc",
		Fcn:         "JoinChain",
		Args:        args,
	}
...
func (c *Resource) extractSignedEnvelope(reqEnvelope []byte) (*context.SignedEnvelope, error) {
	envelope := &common.Envelope{}
	err := proto.Unmarshal(reqEnvelope, envelope)
	if err != nil {
		return nil, errors.Wrap(err, "unmarshal request envelope failed")
	}
	se := context.SignedEnvelope{
...
func (c *Resource) createOrUpdateChannel(txnID context.TransactionID, request context.CreateChannelRequest) error {
	...
	_, err = txn.BroadcastPayload(c.clientContext, payload, []context.Orderer{request.Orderer})
	if err != nil {
...
func (c *Resource) QueryChannels(peer context.ProposalProcessor) (*pb.ChannelQueryResponse, error) {
...
func (c *Resource) QueryInstalledChaincodes(peer context.ProposalProcessor) (*pb.ChaincodeQueryResponse, error) {
...
func (c *Resource) InstallChaincode(req context.InstallChaincodeRequest) ([]*context.TransactionProposalResponse, string, error) {
...
func (c *Resource) queryChaincode(request context.ChaincodeInvokeRequest, targets []context.ProposalProcessor) ([][]byte, error) {
...
func (c *Resource) queryChaincodeWithTarget(request context.ChaincodeInvokeRequest, target context.ProposalProcessor) ([]byte, error) {
	const systemChannel = ""

	targets := []context.ProposalProcessor{target}
...
func validateResponse(response *context.TransactionProposalResponse) error {
