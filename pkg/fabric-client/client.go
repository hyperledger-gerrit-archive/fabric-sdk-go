...
func (c *Client) SaveUserToStateStore(user fab.User, skipPersistence bool) error {
	...
	if !skipPersistence {
		...
		if err != nil {
			return errors.Wrap(err, "stateStore SetValue failed")
		}
	}
	return nil
...
func (c *Client) SignChannelConfig(config []byte) (*common.ConfigSignature, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to get user context's identity")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "signing of channel config failed")
	}
...
func (c *Client) createOrUpdateChannel(request fab.CreateChannelRequest, haveEnvelope bool) error {
	...
	if haveEnvelope {
		...
		if err != nil {
			return errors.Wrap(err, "BuildChannelHeader failed")
		}
		if c.userContext == nil {
		...
		creator, err := c.userContext.Identity()
		if err != nil {
			return errors.Wrap(err, "getting creator failed")
		}
		...
		if err != nil {
			return errors.Wrap(err, "signing payload failed")
		}
	}
	...
	if err != nil {
		return errors.Wrapf(err, "failed broadcast to orderer %s", request.Orderer.URL())
	}
...
func (c *Client) QueryChannels(peer fab.Peer) (*pb.ChannelQueryResponse, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "cscc.GetChannels failed")
	}
...
func (c *Client) QueryInstalledChaincodes(peer fab.Peer) (*pb.ChaincodeQueryResponse, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "lscc.getinstalledchaincodes failed")
	}
...
func (c *Client) InstallChaincode(chaincodeName string, chaincodePath string, chaincodeVersion string,
	...
	if chaincodePackage == nil {
		...
		if err != nil {
			return nil, "", errors.Wrap(err, "PackageCC failed")
		}
	}
...
func (c *Client) queryBySystemChaincodeByTarget(chaincodeID string, fcn string, args []string, target apitxn.ProposalProcessor) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "QueryBySystemChaincode failed")
	}
