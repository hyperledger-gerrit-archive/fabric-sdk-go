...
func testFKVS(t *testing.T, KeySerializer KeySerializer) {
	...
	if err == nil || err != context.ErrNotFound {
...
func checkStoreValue(store context.KVStore, key interface{}, expected []byte) error {
	v, err := store.Load(key)
	if err != nil {
		if err == context.ErrNotFound && expected == nil {
			return nil
		}
		return err
	}
