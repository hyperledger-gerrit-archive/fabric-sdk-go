...
type FabricCAClient interface {
	...
	Reenroll(user context.User) (core.Key, []byte, error)
	Register(registrar context.User, request *RegistrationRequest) (string, error)
	Revoke(registrar context.User, request *RevocationRequest) (*api.RevocationResponse, error)
}
