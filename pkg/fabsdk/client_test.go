...
func TestWithOrgName(t *testing.T) {
	c, err := configImpl.FromFile(clientConfigFile)
	if err != nil {
		t.Fatalf("Unexpected error from config: %v", err)
	}
	...
	_, err = sdk.Client(FromName("notarealuser"), WithOrgName(clientValidExtraOrg))
	if err == nil {
		t.Fatal("Expected error from Client")
	}

	_, err = sdk.Client(FromName(clientValidExtraUser), WithOrgName(clientValidExtraOrg))
	if err != nil {
		t.Fatalf("Expected no error from Client, but got %v", err)
	}
}
