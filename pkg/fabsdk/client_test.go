...
func TestNewGoodClientOpt(t *testing.T) {
	c, err := configImpl.FromFile("testdata/test.yaml")
	if err != nil {
	...
	_, err = sdk.Client(FromUserID("user1"), goodClientOpt())
	if err != nil {
...
func TestNewBadClientOpt(t *testing.T) {
	...
	_, err = sdk.Client(FromUserID("user1"), badClientOpt())
	if err == nil {
...
func TestWithOrg(t *testing.T) {
	...
	_, err = sdk.Client(FromUserID("notarealuser"), WithOrg("orgX"))
	if err == nil {
		t.Fatal("Expected error from Client")
	}

	_, err = sdk.Client(FromUserID("orgXuser"), WithOrg("orgX"))
	if err != nil {
		t.Fatalf("Expected no error from Client, but got %v", err)
	}
}
