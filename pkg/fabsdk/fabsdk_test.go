...
func TestNewGoodOpt(t *testing.T) {
	_, err := New(ConfigFile("../../test/fixtures/config/config_test.yaml"), defPkgSuite(), goodOpt())
	if err != nil {
...
func TestNewBadOpt(t *testing.T) {
	_, err := New(ConfigFile("../../test/fixtures/config/config_test.yaml"), defPkgSuite(), badOpt())
	if err == nil {
...
func TestNewDefaultSDK(t *testing.T) {
	...
	_, err := New(ConfigFile("../../test/fixtures/config/invalid.yaml"), defPkgSuite(), WithStateStorePath("/tmp/state"))
	if err == nil {
		t.Fatalf("Should have failed for invalid config file")
	}

	// Test New SDK with valid config file
	sdk, err := New(ConfigFile("../../test/fixtures/config/config_test.yaml"), defPkgSuite(), WithStateStorePath("/tmp/state"))
	if err != nil {
...
func TestNewDefaultTwoValidSDK(t *testing.T) {
	sdk1, err := New(ConfigFile("../../test/fixtures/config/config_test.yaml"), defPkgSuite(), WithStateStorePath("/tmp/state"))
	if err != nil {
		t.Fatalf("Error initializing SDK: %s", err)
	}

	sdk2, err := New(ConfigFile("./testdata/test.yaml"), defPkgSuite(), WithStateStorePath("/tmp/state"))
	if err != nil {
		t.Fatalf("Error initializing SDK: %s", err)
	}
...
func TestNewDefaultSDKFromByte(t *testing.T) {
	...
	sdk, err := New(ConfigRaw(cBytes, "yaml"), defPkgSuite(), WithStateStorePath("/tmp/state"))
	if err != nil {
		t.Fatalf("Error initializing SDK: %s", err)
	}
	...
	_, err = New(ConfigRaw(cBytes, "json"), defPkgSuite(), WithStateStorePath("/tmp/state"))
	if err == nil {
