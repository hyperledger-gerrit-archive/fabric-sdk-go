...
func TestNewDefaultSDK(t *testing.T) {
	...
	_, err := NewSDK(ConfigFile("../../test/fixtures/config/invalid.yaml"), StateStorePath("/tmp/state"), defImplOption())
	if err == nil {
		t.Fatalf("Should have failed for invalid config file")
	}

	// Test New SDK with valid config file
	sdk, err := NewSDK(ConfigFile("../../test/fixtures/config/config_test.yaml"), StateStorePath("/tmp/state"), defImplOption())
	if err != nil {
...
func TestNewDefaultTwoValidSDK(t *testing.T) {
	sdk1, err := NewSDK(ConfigFile("../../test/fixtures/config/config_test.yaml"), StateStorePath("/tmp/state"), defImplOption())
	if err != nil {
		t.Fatalf("Error initializing SDK: %s", err)
	}

	sdk2, err := NewSDK(ConfigFile("./testdata/test.yaml"), StateStorePath("/tmp/state"), defImplOption())
	if err != nil {
		t.Fatalf("Error initializing SDK: %s", err)
	}
...
func TestNewDefaultSDKFromByte(t *testing.T) {
	...
	sdk, err := NewSDK(ConfigBytes(cBytes, "yaml"), StateStorePath("/tmp/state"), defImplOption())
	if err != nil {
		t.Fatalf("Error initializing SDK: %s", err)
	}
	...
	_, err = NewSDK(ConfigBytes(cBytes, "json"), StateStorePath("/tmp/state"), defImplOption())
	if err == nil {
