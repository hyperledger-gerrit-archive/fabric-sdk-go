...
const (
	...
	validTxClientUser  = "User1"
	validTxClientAdmin = "Admin"
	validTxClientOrg   = "Org2"
)
...
func TestNewChannelMgmtClient(t *testing.T) {
	...
	_, err = sdk.NewChannelMgmtClient(validTxClientAdmin)
	if err != nil {
		t.Fatalf("Failed to create new channel client: %s", err)
	}

	// Test configuration failure for new channel management client with options (invalid org)
	_, err = sdk.NewChannelMgmtClientWithOpts(validTxClientAdmin, &ChannelMgmtClientOpts{OrgName: "Invalid"})
	if err == nil {
	...
	_, err = sdk.NewChannelMgmtClientWithOpts(validTxClientAdmin, &ChannelMgmtClientOpts{OrgName: "ordererorg"})
	if err != nil {
...
func TestNewResourceMgmtClient(t *testing.T) {
	...
	_, err = sdk.NewResourceMgmtClient(validTxClientAdmin)
	if err != nil {
		t.Fatalf("Failed to create new resource management client: %s", err)
	}

	// Test configuration failure for new resource management client with options (invalid org)
	_, err = sdk.NewResourceMgmtClientWithOpts(validTxClientAdmin, &ResourceMgmtClientOpts{OrgName: "Invalid"})
	if err == nil {
	...
	_, err = sdk.NewResourceMgmtClientWithOpts(validTxClientAdmin, &ResourceMgmtClientOpts{OrgName: "Org2"})
	if err != nil {
...
func TestNewPreEnrolledUserSession(t *testing.T) {
	...
	_, err = sdk.NewPreEnrolledUserSession("org1", validTxClientUser)
	if err != nil {
		t.Fatalf("Unexpected error loading user session: %s", err)
	}

	_, err = sdk.NewPreEnrolledUserSession("notarealorg", validTxClientUser)
	if err == nil {
