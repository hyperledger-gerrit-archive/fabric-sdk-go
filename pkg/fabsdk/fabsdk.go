...
import (
	...
	metricsImpl "github.com/hyperledger/fabric-sdk-go/pkg/opconfig"
...
type configs struct {
	...
	metricsConfig     metricsImpl.MetricsConfig
}
...
func WithMetricsConfig(metricsConfigs ...interface{}) Option {
	return func(opts *options) error {
		c, err := metricsImpl.BuildConfigMetricsFromOptions(metricsConfigs...)
		if err != nil {
			return err
		}
...
func initSDK(sdk *FabricSDK, configProvider core.ConfigProvider, opts []Option) error { //nolint
	...
	err = sdk.extraInit(cfg, userStore, signingManager, identityManagerProvider, infraProvider, localDiscoveryProvider, channelProvider)
	if err != nil {
...
func (sdk *FabricSDK) loadMetricsConfig(configBackend ...core.ConfigBackend) (metricsImpl.MetricsConfig, error) {
	metricsConfigOpt, ok := sdk.opts.metricsConfig.(*metricsImpl.OperationsConfigOptions)

	if sdk.opts.metricsConfig == nil || (ok && !metricsImpl.IsMetricsConfigFullyOverridden(metricsConfigOpt)) {
		defMetricsConfig, err := metricsImpl.ConfigFromBackend(configBackend...)
		if err != nil {
		...
		return metricsImpl.UpdateMissingOptsWithDefaultConfig(metricsConfigOpt, defMetricsConfig), nil
	}
