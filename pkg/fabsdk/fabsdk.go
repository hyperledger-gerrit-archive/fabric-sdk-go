...
func (sdk *FabricSDK) loadConfigs(configProvider core.ConfigProvider) (*configs, error) {
	...
	if c.cryptoSuiteConfig == nil || c.endpointConfig == nil || c.identityConfig == nil {
		...
		//cryptoSuiteConfig passed through opts takes priority over default one
...
func (sdk *FabricSDK) loadEndpointConfig(configBackend core.ConfigBackend) (fab.EndpointConfig, error) {
	// for now calling ConfigFromBackend to get default implementation all the time,
	// TODO verify if loading EndpointConfig from opts doesn't implement all or any of the interface functions
	// TODO and if true, only then call ConfigFromBackend and set those functions not provided by opts with defaults
	...
	return fabImpl.UpdateMissingOptsWithDefaultConfig(sdk.opts.endpointConfig.(*fabImpl.EndpointConfigOptions), defEndpointConfig), nil
