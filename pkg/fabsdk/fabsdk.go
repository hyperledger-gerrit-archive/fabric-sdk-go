...
func (sdk *FabricSDK) ChannelContext(options ...ChannelContextOption) contextApi.ChannelProvider {
	...
	channelProvider := func() (contextApi.Channel, error) {
		...
		if channelCtxOpts.channelID == "" {
			return nil, errors.New("failed to create channel context, channel ID not provided")
		}
		...
		return context.NewChannel(clientCtxProvider, channelCtxOpts.channelID)
