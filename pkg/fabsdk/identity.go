...
// NewRegistrarService creates an Registrar Service for the given organization
func (sdk *FabricSDK) NewRegistrarService(orgName string) (identity.RegistrarService, error) {
	im, err := sdk.provider.IdentityProvider().CreateRegistrarService(orgName)
	if err != nil {
		return nil, err
	}
	return im, nil
