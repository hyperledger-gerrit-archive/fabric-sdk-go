...
type FabricProvider interface {
	...
	CreateChannelTransactor(ic identity.Context, cfg apifabclient.ChannelCfg) (apifabclient.Transactor, error)
	CreateEventHub(ic identity.Context, name string) (apifabclient.EventHub, error)

	// deprecated. Use CreateIdentityManager instead
	CreateCAClient(orgID string) (apifabca.FabricCAClient, error)
	CreateIdentityManager(orgID string) (identity.Manager, error)
