...
// FromUserID uses the named user to load the identity
// TODO determine if it's best to call this UserID or EnrollID
func FromUserID(id string) IdentityOption {
	return func(o *identityOptions, sdk *FabricSDK, orgName string) error {
		if o.ok {
			return errors.New("Identity already determined")
		}

		identity, err := sdk.NewPreEnrolledUser(orgName, id)
		if err != nil {
...
// fromIdentity uses a pre-constructed identity object as the credential for the session
// TODO: expose this method?
func fromIdentity(identity fab.User) IdentityOption {
	return func(o *identityOptions, sdk *FabricSDK, orgName string) error {
		if o.ok {
			return errors.New("Identity already determined")
		}
