...
func NewOrgContext(factory apicontext.OrgClientFactory, orgID string, config apiconfig.Config) (*OrgContext, error) {
	c := OrgContext{}
...
type Session struct {
	factory apicontext.SessionClientFactory
	user    fab.User
}

// NewSession creates a session from a context and a user (TODO)
func NewSession(user fab.User, factory apicontext.SessionClientFactory) *Session {
	s := Session{
		factory: factory,
		user:    user,
	}
