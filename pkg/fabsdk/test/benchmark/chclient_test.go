...
func BenchmarkExecuteTx(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		chClient := setupChannelClient(nil, b, enrolledUser)
		...
		chClient = setupChannelClient([]fab.Peer{testPeer1, testPeer2}, b, enrolledUser)
		...
		// finally test successful invoke
		testPeer2.Payload = []byte("test1")
		chClient = setupChannelClient([]fab.Peer{testPeer1, testPeer2}, b, enrolledUser)
		_, err = chClient.Execute(channel.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("b")}})
...
func setupCustomTestContext(b *testing.B, selectionService fab.SelectionService, discoveryService fab.DiscoveryService, orderers []fab.Orderer, signingIdentity mspctx.SigningIdentity) context.ClientProvider {
...
func setupChannelClient(peers []fab.Peer, b *testing.B, signingIdentity mspctx.SigningIdentity) *channel.Client {

	return setupChannelClientWithError(nil, nil, peers, b, signingIdentity)
}

func setupChannelClientWithError(discErr error, selectionErr error, peers []fab.Peer, b *testing.B, signingIdentity mspctx.SigningIdentity) *channel.Client {

	fabCtx := setupCustomTestContext(b, txnmocks.NewMockSelectionService(selectionErr, peers...), txnmocks.NewMockDiscoveryService(discErr), nil, signingIdentity)
