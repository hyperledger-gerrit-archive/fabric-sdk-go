...
func setupChannelClient(b *testing.B, endpointConfig fab.EndpointConfig, ctx context.Client) *channel.Client {
	...
	clntPvdr := setupCustomTestContext(b, nil, endpointConfig, ctx)
...
func setupCustomTestContext(b *testing.B, orderers []fab.Orderer, endpointConfig fab.EndpointConfig, ctx context.Client) context.ClientProvider {
	if orderers == nil {
		orderer := fcmocks.NewMockOrderer("", nil)
