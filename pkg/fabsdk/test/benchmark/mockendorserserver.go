...
func (m *MockEndorserServer) ProcessProposal(context context.Context,
	...
	if err != nil {
		return nil, errors.Wrap(errors.GeneralError, err, "Error getting function name from mockPeer")
	}
...
func (p *MockPeer) ProcessTransactionProposal(tp fab.TransactionProposal, funcName []byte) (*fab.TransactionProposalResponse, error) {
	...
	if err != nil {
		return nil, errors.Wrap(errors.GeneralError, err, "Error received while parsing EC Key")
	}
	proposalResponsePayload, err := p.createProposalResponsePayload()
	if err != nil {
		return nil, errors.Wrap(errors.GeneralError, err, "Error received while creating proposal response")
	}
	sigma, err := SignECDSA(lowLevelKey, append(proposalResponsePayload, p.Endorser...))
	if err != nil {
		return nil, errors.Wrap(errors.GeneralError, err, "Error received while signing proposal for endorser with EC key")
	}
