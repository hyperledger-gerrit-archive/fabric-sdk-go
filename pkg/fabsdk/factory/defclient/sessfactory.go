...
func (f *SessionClientFactory) NewChannelMgmtClient(providers apisdk.Providers, session apisdk.SessionSvc) (chmgmt.ChannelMgmtClient, error) {
	// For now settings are the same as for system client
	client, err := providers.FabricProvider().NewResourceClient(session.Identity())
	if err != nil {
		return nil, err
	}
	return chmgmtImpl.NewChannelMgmtClient(providers, session.Identity(), client)
}

// NewResourceMgmtClient returns a client that manages resources
func (f *SessionClientFactory) NewResourceMgmtClient(providers apisdk.Providers, session apisdk.SessionSvc, filter resmgmt.TargetFilter) (resmgmt.ResourceMgmtClient, error) {
	...
	return resmgmtImpl.NewResourceMgmtClient(providers, session.Identity(), client, session, discovery, filter)
}
...
func (f *SessionClientFactory) NewChannelClient(providers apisdk.Providers, session apisdk.SessionSvc, channelID string) (apitxn.ChannelClient, error) {
	...
	return chImpl.NewChannelClient(providers, resource, channel, discovery, selection, eventHub)
}
