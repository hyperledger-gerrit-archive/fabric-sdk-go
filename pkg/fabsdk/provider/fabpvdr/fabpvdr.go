...
func New(config core.Config, opts ...options.Opt) *InfraProvider {
	...
	chConfigRefreshTime := config.TimeoutOrDefault(core.ChannelConfigRefresh)
	pvdr := &InfraProvider{}
	...
	pvdr.commManager = comm.NewCachingConnector(sweepTime, idleTime)
	pvdr.eventServiceCache = eventServiceCache
	pvdr.chCfgCache = chconfig.NewRefCache(chConfigRefreshTime, pvdr.CreateChannelConfig)
...
func (f *InfraProvider) CreateChannelMembership(ctx fab.ClientContext, channel string) (fab.ChannelMembership, error) {
	refresh := f.providerContext.Config().TimeoutOrDefault(core.ChannelMembershipRefresh)
	chCfgRef, err := f.loadChannelCfgRef(ctx, channel)
	if err != nil {
		return nil, err
	}

	return membership.NewRef(refresh, membership.Context{Providers: f.providerContext}, chCfgRef.Reference), nil
}
...
func (f *InfraProvider) loadChannelCfgRef(ctx fab.ClientContext, channelID string) (*chconfig.Ref, error) {
	key, err := chconfig.NewCacheKey(ctx, channelID)
	if err != nil {
		return nil, err
	}
