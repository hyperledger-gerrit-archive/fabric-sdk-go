...
func New(config fab.EndpointConfig) *InfraProvider {
	...
	discoverIdleTime := config.Timeout(fab.DiscoverServiceIdle)
	...
	discoverServiceCache := lazycache.New(
		...
		func(key lazycache.Key) (interface{}, error) {
			ck := key.(cacheKey)
			return getDiscoverClient(ck.Context(), ck.ChannelConfig(), discoverIdleTime, ck.Opts()...)
		},
	)
...
func (f *InfraProvider) CreateDiscoverService(ctx fab.ClientContext, channelID string, opts ...options.Opt) (fab.DiscoverService, error) {
	chnlCfg, err := f.CreateChannelCfg(ctx, channelID)
	if err != nil {
		return nil, err
	}
...
func getDiscoverClient(ctx context.Client, chConfig fab.ChannelCfg, idleTime time.Duration, opts ...options.Opt) (*discover.Client, error) {
	...
	return discover.New(ctx, chConfig, peerURL, idleTime, opts...)
}
