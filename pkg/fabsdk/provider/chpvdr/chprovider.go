...
func (cp *ChannelProvider) getDiscoveryService(context fab.ClientContext, channelID string) (fab.DiscoveryService, error) {
	...
	discoveryServiceRef, err := cp.discoveryServiceCache.Get(key)
	if err != nil {
		return nil, err
	}
	discoveryService, err := discoveryServiceRef.(*lazyref.Reference).Get()
	if err != nil {
		return nil, err
	}
...
func (cp *ChannelProvider) createSelectionService(ctx context.Client, chConfig fab.ChannelCfg) (fab.SelectionService, error) {
	...
	// FIXME: Pass in users/or get rid of users
	selection, err := dynamicselection.NewService(ctx, chConfig.ID(), discovery,
		dynamicselection.WithLoadBalancePolicy(pgresolver.NewRandomLBP()),
		dynamicselection.WithCacheTimeout(5*time.Second)) // FIXME: Get from config
	if err != nil {
		return nil, err
	}
...
func (cp *ChannelProvider) getSelectionService(context fab.ClientContext, channelID string) (fab.SelectionService, error) {
	...
	selectionServiceRef, err := cp.selectionServiceCache.Get(key)
	if err != nil {
		return nil, err
	}
	selectionService, err := selectionServiceRef.(*lazyref.Reference).Get()
	if err != nil {
		return nil, err
	}
