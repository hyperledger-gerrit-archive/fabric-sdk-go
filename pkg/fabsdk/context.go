...
func (c *sdkContext) ChannelProvider() *chpvdr.ChannelProvider {
...
func newSession(ic apifabclient.IdentityContext, cp *chpvdr.ChannelProvider) *session {
	s := session{
		identityContext: ic,
...
func (s *session) EventHub(channelID string) (apifabclient.EventHub, error) {
	return s.channelService.EventHub(channelID)
}
