...
func TestDefLoggerFactory(t *testing.T) {
	...
	_, err := New(withConfigFile("../../test/fixtures/config/config_test.yaml"), fromPkgSuite())
	if err != nil {
...
func TestOptLoggerFactory(t *testing.T) {
	...
	_, err := New(withConfigFile("../../test/fixtures/config/config_test.yaml"), dePkgSuiteWithLogger(lf))
	if err != nil {
		t.Fatalf("Error initializing SDK: %s", err)
	}
