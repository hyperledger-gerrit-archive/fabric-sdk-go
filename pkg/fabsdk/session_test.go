...
func TestFromUserIDValid(t *testing.T) {
	...
	opt := FromUserID(validIdentityOptUser)
...
func TestFromUserIDInvalid(t *testing.T) {
	c, err := configImpl.FromFile(identityOptConfigFile)
	if err != nil {
		t.Fatalf("Unexpected error from config: %v", err)
	}
...
func TestFromIdentity(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Unexpected error from loading user: %v", err)
	}
