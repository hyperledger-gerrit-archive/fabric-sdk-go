...
func TestBroadcastPayloadWithOrdererDialFailure(t *testing.T) {
	...
	require.NoError(t, err, "BroadCastEnvelope to running orders returned a connection error")

	// stop orderer2 and try again (orderer1 and orderer3 should successfully connect)
	orderer2.Stop()
	_, err = broadcastEnvelope(parentCtx, sigEnvelope, orderers)
	require.NoError(t, err, "BroadCastEnvelope to running order1 and orderer3 returned a connection error")

	// stop orderer1 and try again (only orderer3 should successfully connect)
	orderer1.Stop()
	_, err = broadcastEnvelope(parentCtx, sigEnvelope, orderers)
	require.NoError(t, err, "BroadCastEnvelope to running order3 returned a connection error")
	...
	defer cancel2()
	_, err = broadcastEnvelope(parentCtx, sigEnvelope, orderers)
	require.Error(t, err, "BroadCastEnvelope to running orders returned no error with 1 nano second context deadline")
