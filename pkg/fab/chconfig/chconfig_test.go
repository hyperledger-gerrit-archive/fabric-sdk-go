...
func TestChannelConfigWithPeerWithRetries(t *testing.T) {
	...
	chConfig := &core.ChannelConfig{
		Policies: core.ChannelPolicies{QueryChannel: core.QueryChannelConfigPolicy{
			MinResponses: 2,
			MaxTargets:   1,
	...
	channelConfig.opts.RetryHandler = &customRetryHandler{handler: retryHandler, retries: 0}
	...
	assert.True(t, channelConfig.opts.RetryHandler.(*customRetryHandler).retries-1 == numberOfAttempts, "number of attempts missmatching")
}
...
func TestResolveOptsFromConfig(t *testing.T) {
	...
	assert.True(t, channelConfig.opts.RetryHandler == nil, "supposed to be nil when not resolved")
	...
	assert.True(t, channelConfig.opts.RetryHandler != nil, "supposed to be loaded once opts resolved from config")
