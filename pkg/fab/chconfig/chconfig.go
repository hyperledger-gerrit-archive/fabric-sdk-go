...
type Opts struct {
	...
	MinTargets   int         //if configured, channel config will be retrieved for these number of random targets
}
...
func (c *ChannelConfig) queryPeers(reqCtx reqContext.Context) (*ChannelCfg, error) {
	...
	if c.opts.Targets == nil {
		...
		targets = randomMinTargets(targets, c.opts.MinTargets)
...
// WithMinTargets encapsulates minTargets to Option
func WithMinTargets(minTargets int) Option {
	return func(opts *Opts) error {
...
func prepareOpts(options ...Option) (Opts, error) {
	...
	if opts.MinTargets == 0 {
		opts.MinTargets = defaultMinTargets
	}
...
//randomMinTargets returns random sub set of min length targets
func randomMinTargets(targets []fab.ProposalProcessor, min int) []fab.ProposalProcessor {
	if len(targets) <= min {
		return targets
	}
	for i := range targets {
