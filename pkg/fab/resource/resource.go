...
func SignChannelConfig(clientContext context.Client, config []byte, signer context.Identity) (*common.ConfigSignature, error) {
...
func CreateChannel(clientContext context.Client, request api.CreateChannelRequest) (fab.TransactionID, error) {
	...
	if request.Envelope != nil {
		return createChannelFromEnvelope(clientContext, request)
	}
	...
	txh, err := txn.NewHeader(clientContext, request.Name)
	if err != nil {
		return fab.EmptyTransactionID, errors.WithMessage(err, "creation of transaction header failed")
	}

	return txh.TransactionID(), createOrUpdateChannel(clientContext, txh, request)
}

// TODO: this function was extracted from createOrUpdateChannel, but needs a closer examination.
func createChannelFromEnvelope(clientContext context.Client, request api.CreateChannelRequest) (fab.TransactionID, error) {
	...
	// Send request
	reqCtx, cancel := reqContext.WithTimeout(contextImpl.NewRequest(clientContext), clientContext.Config().TimeoutOrDefault(core.OrdererResponse))
...
func GenesisBlockFromOrderer(clientContext context.Client, channelName string, orderer fab.Orderer) (*common.Block, error) {
	return retrieveBlock(clientContext, []fab.Orderer{orderer}, channelName, newSpecificSeekPosition(0))
}

// LastConfigFromOrderer fetches the current configuration block for the specified channel
// from the given orderer
func LastConfigFromOrderer(clientContext context.Client, channelName string, orderer fab.Orderer) (*common.ConfigEnvelope, error) {
	logger.Debugf("channelConfig - start for channel %s", channelName)

	// Get the newest block
	block, err := retrieveBlock(clientContext, []fab.Orderer{orderer}, channelName, newNewestSeekPosition())
	if err != nil {
		return nil, err
	}
	...
	block, err = retrieveBlock(clientContext, []fab.Orderer{orderer}, channelName, newSpecificSeekPosition(lastConfig.Index))
	if err != nil {
...
func JoinChannel(clientContext context.Client, request api.JoinChannelRequest) error {
	...
	_, err = queryChaincode(clientContext, cir, request.Targets)
	return err
}
...
func createOrUpdateChannel(clientContext context.Client, txh *txn.TransactionHeader, request api.CreateChannelRequest) error {
	...
	channelHeaderOpts := txn.ChannelHeaderOpts{
		TxnHeader:   txh,
		TLSCertHash: ccomm.TLSCertHash(clientContext.Config()),
	}
	...
	_, err = txn.BroadcastPayload(clientContext, payload, []fab.Orderer{request.Orderer})
	if err != nil {
...
func QueryChannels(clientContext context.Client, peer fab.ProposalProcessor) (*pb.ChannelQueryResponse, error) {
	...
	payload, err := queryChaincodeWithTarget(clientContext, cir, peer)
	if err != nil {
...
func QueryInstalledChaincodes(clientContext context.Client, peer fab.ProposalProcessor) (*pb.ChaincodeQueryResponse, error) {
...
func InstallChaincode(clientContext context.Client, req api.InstallChaincodeRequest) ([]*fab.TransactionProposalResponse, fab.TransactionID, error) {
	...
	txh, err := txn.NewHeader(clientContext, fab.SystemChannel)
	if err != nil {
	...
	transactionProposalResponse, err := txn.SendProposal(clientContext, prop, req.Targets)

	return transactionProposalResponse, prop.TxnID, err
}

func queryChaincode(clientContext context.Client, request fab.ChaincodeInvokeRequest, targets []fab.ProposalProcessor) ([][]byte, error) {
	var errors multi.Errors
	responses := [][]byte{}

	for _, target := range targets {
		resp, err := queryChaincodeWithTarget(clientContext, request, target)
...
func queryChaincodeWithTarget(clientContext context.Client, request fab.ChaincodeInvokeRequest, target fab.ProposalProcessor) ([]byte, error) {
	...
	tpr, err := txn.SendProposal(clientContext, tp, targets)
	if err != nil {
