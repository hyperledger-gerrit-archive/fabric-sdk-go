...
func retrieveBlock(clientContext context.Client, orderers []fab.Orderer, channel string, pos *ab.SeekPosition) (*common.Block, error) {
	th, err := txn.NewHeader(clientContext, channel)
	if err != nil {
	...
	return txn.SendPayload(clientContext, &payload, orderers)
}
