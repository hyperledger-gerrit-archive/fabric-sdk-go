	...
	for _, tx := range fblock.FilteredTransactions {
		...
		if tx.TxValidationCode == pb.TxValidationCode_VALID {
			...
			if len(txActions.ChaincodeActions) == 0 {
				logger.Warnf("No chaincode action found for TxID[%s], block[%d], source URL[%s]", tx.Txid, fblock.Number, sourceURL)
			}
			...
			logger.Warnf("Cannot publish CCEvents for block[%d] and source URL[%s] since Tx Validation Code[%d] is not valid", fblock.Number, sourceURL, tx.TxValidationCode)
		}
	}
}
