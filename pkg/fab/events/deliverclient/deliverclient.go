...
func (c *Client) setSeekFromLastBlockReceived() error {
	...
	if lastBlockNum < math.MaxUint64 {
		...
		logger.Infof("Setting seek info from last block received + 1: %d", c.fromBlock)
	} else {
		// We haven't received any blocks yet. Just ask for the newest
		logger.Infof("Setting seek info from newest")
...
func (c *Client) seekInfo() (*ab.SeekInfo, error) {
	...
	switch c.seekType {
	case seek.Newest:
		logger.Infof("Returning seek info: Newest")
		return seek.InfoNewest(), nil
	case seek.Oldest:
		logger.Infof("Returning seek info: Oldest")
		return seek.InfoOldest(), nil
	case seek.FromBlock:
		logger.Infof("Returning seek info: FromBlock(%d)", c.fromBlock)
