...
func TestInvalidOptionsInNewClient(t *testing.T) {
	// Filtered Client
	if _, err := NewFiltered(newMockContext(), "", clientmocks.NewDiscoveryService(peer1, peer2)); err == nil {
		t.Fatalf("expecting error with no channel ID but got none")
	}
	// Client
	if _, err := New(newMockContext(), "", clientmocks.NewDiscoveryService(peer1, peer2)); err == nil {
		t.Fatalf("expecting error with no channel ID but got none")
	}
}
...
type ClientProvider func(context fabcontext.Context, channelID string, connectionProvider api.ConnectionProvider, discoveryService fab.DiscoveryService, opts ...options.Opt) (*Client, error)

var clientProvider = func(context fabcontext.Context, channelID string, connectionProvider api.ConnectionProvider, discoveryService fab.DiscoveryService, opts ...options.Opt) (*Client, error) {
	return newClient(context, channelID, connectionProvider, discoveryService, true, opts...)
}

var filteredClientProvider = func(context fabcontext.Context, channelID string, connectionProvider api.ConnectionProvider, discoveryService fab.DiscoveryService, opts ...options.Opt) (*Client, error) {
	return newClient(context, channelID, connectionProvider, discoveryService, false, opts...)
}

func newClientWithMockConn(context fabcontext.Context, channelID string, connectionProvider api.ConnectionProvider, clientProvider ClientProvider, discoveryService fab.DiscoveryService, connOpts ...clientmocks.Opt) (*Client, *delivermocks.MockConnection, error) {
	conn := delivermocks.NewConnection(connOpts...)
	client, _, err := newClientWithMockConnAndOpts(
		context, channelID,
		clientmocks.NewProviderFactory().Provider(conn),
		clientProvider, discoveryService,
		defaultOpts,
	)
	return client, conn, err
}

func newClientWithMockConnAndOpts(context fabcontext.Context, channelID string, connectionProvider api.ConnectionProvider, clientProvider ClientProvider, discoveryService fab.DiscoveryService, opts []options.Opt, connOpts ...clientmocks.Opt) (*Client, *delivermocks.MockConnection, error) {
	var conn *delivermocks.MockConnection
