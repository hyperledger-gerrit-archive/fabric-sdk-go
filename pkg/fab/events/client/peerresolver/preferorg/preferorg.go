...
func (r *PeerResolver) ShouldDisconnect(peers []fab.Peer, connectedPeer fab.Peer) bool {
	if connectedPeer.MSPID() != r.mspID {
		...
		for _, p := range r.blockHeightResolver.Filter(peers) {
			if p.MSPID() == r.mspID {
				logger.Debugf("The block height of peer [%s] is greater than or equal to the threshold so the event client will be disconnected from the peer in the other org [%s]", p.URL(), connectedPeer.URL())
				return true
			}

			logger.Debugf("The block height of peer [%s] is less than the cutoff so the event client will NOT be disconnected from the peer in the other org [%s]", p.URL(), connectedPeer.URL())
		}
	}
