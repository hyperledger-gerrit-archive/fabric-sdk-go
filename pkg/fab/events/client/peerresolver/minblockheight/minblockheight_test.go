...
func TestFilter(t *testing.T) {
	...
	resolver = New(dispatcher, ctx, WithBlockHeightLagThreshold(5))
	filteredPeers = resolver.Filter(peers)
	assert.Equal(t, 2, len(filteredPeers))

	resolver = New(dispatcher, ctx, WithBlockHeightLagThreshold(20))
	filteredPeers = resolver.Filter(peers)
	assert.Equal(t, 3, len(filteredPeers))
...
func TestShouldDisconnect(t *testing.T) {
	...
	resolver := New(dispatcher, ctx, WithReconnectBlockHeightThreshold(120))
	...
	resolver = New(dispatcher, ctx, WithReconnectBlockHeightThreshold(5))
	disconnect = resolver.ShouldDisconnect(peers, p1)
