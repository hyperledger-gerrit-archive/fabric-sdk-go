...
func TestResolve(t *testing.T) {
	...
	resolver = New(dispatcher, ctx, []string{p1O1.URL()}, minblockheight.WithBlockHeightLagThreshold(5))
	peer, err = resolver.Resolve(peers)
	require.NoError(t, err)
...
func TestShouldDisconnect(t *testing.T) {
	...
	resolver := New(dispatcher, ctx, []string{p2O1.URL(), p2O1.URL()}, minblockheight.WithBlockHeightLagThreshold(5))
	...
	resolver = New(dispatcher, ctx, []string{p2O1.URL()}, minblockheight.WithBlockHeightLagThreshold(1))
	disconnect = resolver.ShouldDisconnect(peers, p2O2)
