...
var ehConnProvider = func(channelID string, context context.Context, peer fab.Peer) (api.Connection, error) {
	...
	return connection.New(
		...
		comm.WithFailFast(eventEndpoint.FailFast),
		comm.WithCertificate(eventEndpoint.Certificate),
		comm.WithHostOverride(eventEndpoint.HostOverride),
		comm.WithKeepAliveParams(eventEndpoint.KeepAliveParams),
		comm.WithConnectTimeout(eventEndpoint.ConnectTimeout),
	)
}
