	...
	eventClient, _, err := newClientWithMockConnAndOpts(
		...
		clientmocks.NewProviderFactory().Provider(
			ehclientmocks.NewConnection(
				clientmocks.WithLedger(ehclientmocks.NewMockLedger(ehclientmocks.BlockEventFactory)),
			)),
...
func testReconnect(t *testing.T, reconnect bool, maxReconnectAttempts uint, expectedOutcome clientmocks.Outcome, connAttemptResult clientmocks.ConnectAttemptResults) {
	...
	ledger := ehclientmocks.NewMockLedger(ehclientmocks.BlockEventFactory)
	...
	eventClient, _, err := newClientWithMockConnAndOpts(
		...
		newOpts(
			...
			client.WithConnectEventCh(connectch),
