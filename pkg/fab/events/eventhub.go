...
type eventClientFactory interface {
	newEventsClient(provider context.Provider, identity context.Identity, peerAddress string, certificate *x509.Certificate, serverHostOverride string, regTimeout time.Duration, adapter cnsmr.EventAdapter, kap keepalive.ClientParameters, failFast bool, allowInsecure bool) (fab.EventsClient, error)
}

// consumerClientFactory is the default implementation oif the eventClientFactory
type consumerClientFactory struct{}

func (ccf *consumerClientFactory) newEventsClient(provider context.Provider, identity context.Identity, peerAddress string, certificate *x509.Certificate, serverHostOverride string,
