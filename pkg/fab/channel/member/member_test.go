...
func TestNewMemberID(t *testing.T) {
	...
	m, err := New(Context{ProviderContext: ctx}, cfg)
	assert.NotNil(t, err)
	assert.Nil(t, m)
	...
	sID := &msp.SerializedIdentity{Mspid: goodMSPID, IdBytes: []byte(certPem)}
	goodEndorser, err := proto.Marshal(sID)
	assert.Nil(t, err)

	sID = &msp.SerializedIdentity{Mspid: badMSPID, IdBytes: []byte(certPem)}
