...
func (mgr *IdentityManager) GetSigningIdentity(userName string) (*api.SigningIdentity, error) {
	...
	if mgr.userStore != nil {
		user, err := mgr.userStore.Load(api.UserKey{MspID: mgr.mspID, Name: userName})
		if err == nil {
...
func (mgr *IdentityManager) createSigningIdentity(user api.User) (*fabric_ca.Identity, error) {
	// Validate user
	if user == nil {
	...
	return mgr.caClient.NewIdentity(key, cert)
}
