...
func (m *MockBroadcastServer) Broadcast(server po.AtomicBroadcast_BroadcastServer) error {
	...
	m.mockBlockDelivery(chdr)

	return nil
}

func (m *MockBroadcastServer) mockBlockDelivery(channelHeader *common.ChannelHeader) {
	if m.Deliveries != nil {
		block := mocks.NewBlock(channelHeader.ChannelId,
			mocks.NewTransaction(channelHeader.TxId, pb.TxValidationCode_VALID, common.HeaderType_MESSAGE),
		)
		// m.blkNum is used by FilteredBlock only

		m.Deliveries <- block
	} else if m.FilteredDeliveries != nil {
		filteredBlock := mocks.NewFilteredBlock(channelHeader.ChannelId,
			mocks.NewFilteredTx(channelHeader.TxId, pb.TxValidationCode_VALID),
		)
