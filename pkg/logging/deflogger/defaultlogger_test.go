...
func testDefaultLogging(t *testing.T) {
	...
	logger := GetDefaultLogger(moduleName)

	//change the output to buffer
	var buf bytes.Buffer
	logger.(*DefaultLogger).defaultLogger.SetOutput(&buf)
	...
	*/
	...
	//Should be false
	utils.VerifyFalse(t, IsEnabledFor(moduleName, apilogging.DEBUG), "apilogging.IsEnabled for is not working as expected, expected false but got true")

	//Now change the log level to apilogging.DEBUG
	SetLevel(moduleName, apilogging.DEBUG)

	//Should be false
	utils.VerifyTrue(t, IsEnabledFor(moduleName, apilogging.DEBUG), "apilogging.IsEnabled for is not working as expected, expected true but got false")
