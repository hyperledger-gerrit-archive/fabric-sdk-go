...
func TestTxProposalResponseFilter(t *testing.T) {
	...
	_, err := chClient.Query(InvokeRequest{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err == nil {
...
func TestQuery(t *testing.T) {
	...
	_, err := chClient.Query(InvokeRequest{})
	if err == nil {
		t.Fatalf("Should have failed for empty query request")
	}

	_, err = chClient.Query(InvokeRequest{Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err == nil {
		t.Fatalf("Should have failed for empty chaincode ID")
	}

	_, err = chClient.Query(InvokeRequest{ChaincodeID: "testCC", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err == nil {
		t.Fatalf("Should have failed for empty function")
	}

	response, err := chClient.Query(InvokeRequest{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err != nil {
...
func TestQueryWithOptAsync(t *testing.T) {
	...
	go func() {
		resp, err := chClient.Query(InvokeRequest{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
...
func TestQueryWithOptTarget(t *testing.T) {
	...
	response, err := chClient.Query(InvokeRequest{ChaincodeID: "testCC", Fcn: "invoke",
...
func TestExecuteTx(t *testing.T) {
	...
	_, err := chClient.Execute(InvokeRequest{})
	if err == nil {
		t.Fatalf("Should have failed for empty invoke request")
	}

	_, err = chClient.Execute(InvokeRequest{Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	if err == nil {
		t.Fatalf("Should have failed for empty chaincode ID")
	}

	_, err = chClient.Execute(InvokeRequest{ChaincodeID: "testCC", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	if err == nil {
		t.Fatalf("Should have failed for empty function")
	}
	...
	testPeer2.Payload = []byte("test2")
	chClient = setupChannelClient([]fab.Peer{testPeer1, testPeer2}, t)
	_, err = chClient.Execute(InvokeRequest{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("b")}})
	if err == nil {
		t.Fatalf("Should have failed")
	}
	s, ok := status.FromError(err)
...
func TestInvokeHandler(t *testing.T) {
	...
	response, err := chClient.InvokeHandler(handler, InvokeRequest{ChaincodeID: "testCC", Fcn: "move", Args: [][]byte{[]byte("a"), []byte("b"), []byte("1")}})
	if err != nil {
...
func TestQueryWithCustomEndorser(t *testing.T) {
	...
	response, err := chClient.InvokeHandler(
		...
		InvokeRequest{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}},
	)
	if err != nil {
		t.Fatalf("Failed to invoke test cc: %s", err)
	}
...
func TestExecuteTxDiscoveryError(t *testing.T) {
	...
	_, err := chClient.Execute(InvokeRequest{ChaincodeID: "testCC", Fcn: "invoke",
...
func TestOrdererStatusError(t *testing.T) {
	...
	_, err := chClient.Execute(InvokeRequest{ChaincodeID: "test", Fcn: "invoke",
		Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	statusError, ok := status.FromError(err)
...
func TestTransactionValidationError(t *testing.T) {
	...
	response, err := chClient.Execute(InvokeRequest{ChaincodeID: "test", Fcn: "invoke",
		Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
