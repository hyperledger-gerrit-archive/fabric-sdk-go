...
func (e *SelectAndEndorseHandler) Handle(requestContext *RequestContext, clientContext *ClientContext) {
	...
	if len(targets) == 0 {
		var err error
		ccCalls, requestContext.Opts.Targets, err = getEndorsers(requestContext, clientContext, nil)
		if err != nil {
...
func getEndorsers(requestContext *RequestContext, clientContext *ClientContext, retryOpts *retry.Opts) ([]*fab.ChaincodeCall, []fab.Peer, error) {
	...
	if retryOpts != nil {
		selectionOpts = append(selectionOpts, selectopts.WithRetryOpts(*retryOpts))
	}
...
func getAdditionalEndorsers(requestContext *RequestContext, clientContext *ClientContext, invocationChain []*fab.ChaincodeCall) ([]fab.Peer, error) {
	...
	_, endorsers, err := getEndorsers(requestContext, clientContext, &retry.Opts{})
	if err != nil {
...
func mergeCCCall(invocChain []*fab.ChaincodeCall, respCCCall *fab.ChaincodeCall) (*fab.ChaincodeCall, bool) {
	ccCall := getCCCall(invocChain, respCCCall.ID)
	if ccCall != nil {
...
func getCCCall(invocChain []*fab.ChaincodeCall, ccID string) *fab.ChaincodeCall {
