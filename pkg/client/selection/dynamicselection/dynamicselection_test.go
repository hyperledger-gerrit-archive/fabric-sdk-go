...
func TestGetEndorsersForChaincodeOneCC(t *testing.T) {
	...
	channelPeers := []context.Peer{p1, p2, p3, p4, p5, p6, p7, p8}
...
func TestGetEndorsersForChaincodeTwoCCsTwoChannels(t *testing.T) {
	...
	channel1Peers := []context.Peer{p1, p2, p3, p4, p5, p6, p7, p8}
	...
	channel2Peers := []context.Peer{p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12}
	verify(t, service, expected, channel2, channel2Peers, cc1, cc2)
}

func verify(t *testing.T, service context.SelectionService, expectedPeerGroups []pgresolver.PeerGroup, channelID string, channelPeers []context.Peer, chaincodeIDs ...string) {
	// Set the log level to WARNING since the following spits out too much info in DEBUG
	module := "pg-resolver"
	level := logging.GetLevel(module)
	logging.SetLevel(module, api.WARNING)
...
func containsPeerGroup(groups []pgresolver.PeerGroup, peers []context.Peer) bool {
...
func containsAllPeers(peers []context.Peer, pg pgresolver.PeerGroup) bool {
...
func containsPeer(peers []context.Peer, peer context.Peer) bool {
	for _, p := range peers {
		if p.URL() == peer.URL() {
			return true
		}
...
func pg(peers ...context.Peer) pgresolver.PeerGroup {
	return pgresolver.NewPeerGroup(peers...)
}

func peer(name string, mspID string) context.Peer {
...
func newMockSelectionService(ccPolicyProvider CCPolicyProvider, lbp pgresolver.LoadBalancePolicy) context.SelectionService {
...
func toString(peers []context.Peer) string {
	str := "["
	for i, p := range peers {
		str += p.URL()
		if i+1 < len(peers) {
...
func TestDynamicSelection(t *testing.T) {
	...
	chPeers := []context.Peer{peer("p0", "Org1")}
...
type DynamicSelectionProviderFactory struct {
	...
	selectionProvider context.SelectionProvider
}
