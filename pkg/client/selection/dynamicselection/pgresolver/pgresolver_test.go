...
var peersByMSPID = map[string][]context.Peer{
	org1: peers(p1, p2),
	org2: peers(p3, p4),
	org3: peers(p5, p6, p7),
...
func peer(name string) context.Peer {
	mp := mocks.NewMockPeer(name, "localhost:7051")
	return mp
}

func peers(peers ...context.Peer) []context.Peer {
	return peers
}
...
func pg(peers ...context.Peer) PeerGroup {
	return NewPeerGroup(peers...)
}

func retrievePeersByMSPid(mspID string) []context.Peer {
...
func containsPeer(pg []context.Peer, p context.Peer) bool {
	for _, peer := range pg {
		if peer.Name() == p.Name() {
			return true
		}
