...
func (s *selectionService) getPeerGroupResolver(channelPeers []context.Peer, chaincodeIDs []string) (pgresolver.PeerGroupResolver, error) {
...
func (s *selectionService) createPGResolver(channelPeers []context.Peer, key *resolverKey) (pgresolver.PeerGroupResolver, error) {
	s.mutex.Lock()
...
func (s *selectionService) getPolicyGroupForCC(channelID string, ccID string, channelPeers []context.Peer) (pgresolver.Group, error) {
	...
	return pgresolver.NewSignaturePolicyCompiler(
		func(mspID string) []context.Peer {
			return s.getAvailablePeers(channelPeers, mspID)
		}).Compile(sigPolicyEnv)
}

func (s *selectionService) getAvailablePeers(channelPeers []context.Peer, mspID string) []context.Peer {
	var peers []context.Peer
