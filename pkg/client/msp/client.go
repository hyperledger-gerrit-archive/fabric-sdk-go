...
type Client struct {
	...
	caID string
	// Default CA name. If caName parameter is not specified in a call to a CA, this value will be used (optional)
	caName string
	ctx    context.Client
}
...
func New(clientProvider context.ClientProvider, opts ...ClientOption) (*Client, error) {
	...
	ctx, msp, err := initClientFromOptions(clientProvider, opts...)
	if err != nil {
		return nil, err
	}
	...
	if msp.caID == "" && len(org.CertificateAuthorities) > 0 {
		// Default to the first CA in org, if it is defined
		msp.caID = org.CertificateAuthorities[0]
	}
	if msp.caID != "" {
		ca, ok := ctx.IdentityConfig().CAConfig(msp.caID)
		if !ok {
			return nil, fmt.Errorf("non-existent CA: '%s'", msp.caID)
		}
		msp.caName = ca.CAName
	}

	err = veiifyOrgCA(org, msp.caID)
	if err != nil {
		return nil, err
	}
...
func (c *Client) GetAllIdentities(opts ...RequestOption) ([]*IdentityResponse, error) {
	...
	o, err := prepareOptsFromOptions(opts...)
	if err != nil {
		return nil, err
	}
...
func prepareOptsFromOptions(opts ...RequestOption) (options, error) {
	o := options{}
