...
func asPeers(ctx contextAPI.Client, endpoints []*discclient.Peer) ([]fab.Peer, error) {
	var peers []fab.Peer
	for _, endpoint := range endpoints {
		...
		peer, err := ctx.InfraProvider().CreatePeerFromConfig(&fabApi.NetworkPeer{PeerConfig: *peerConfig, MSPID: endpoint.MSPID})
		if err != nil {
			logger.Warnf("Unable to get peer config for [%s]: %s", url, err)
			continue
		}
