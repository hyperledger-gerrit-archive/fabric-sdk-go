...
func NewChannelService(ctx contextAPI.Client, channelID string, opts ...coptions.Opt) (*ChannelService, error) {
	...
	err := s.service.Initialize(ctx) // FIXME: Should get rid of Initialize
	if err != nil {
		return nil, err
	}
	return s, nil
...
func (s *ChannelService) getTargets(ctx contextAPI.Client) ([]fab.PeerConfig, error) {
	...
	chpeers, err := ctx.EndpointConfig().ChannelPeers(s.channelID)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to get peer configs for channel [%s]", s.channelID)
	}
