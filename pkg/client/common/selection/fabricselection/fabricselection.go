...
// PeerEndpoint is a Peer that also includes the block (ledger) height
...
func (s *Service) queryEndorsers(chaincodes []*fab.ChaincodeCall) (discclient.ChannelResponse, error) {
	...
	for i := 0; i < s.maxRetries; i++ {
		var chResp discclient.ChannelResponse
		chResp, lastErr = s.query(req, chaincodes, targets)
		if lastErr == nil {
			return chResp, nil
		}

		if !strings.Contains(lastErr.Error(), "failed constructing descriptor for chaincodes") {
			logger.Debugf("Received error: %s", lastErr)
			return nil, lastErr
		}

		// This may be a transient error where blocks have not yet been propagated to all peers.
		logger.Debugf("Received transient error: %s. Retrying in %s", lastErr, s.retryInterval)
...
func asPeer(ctx contextAPI.Client, endpoint *discclient.Peer) (PeerEndpoint, error) {
