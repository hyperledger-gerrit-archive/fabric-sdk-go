...
func (dp *ccPolicyProvider) queryChaincode(ccID string, ccFcn string, ccArgs [][]byte) ([]byte, error) {
	...
	//TODO Need to get channel context here
	chanelContext, err := dp.getChannelContext()
	if err != nil {
		return nil, errors.WithMessage(err, "Unable to create channel context")
	}

	client, err := channel.New(chanelContext)
	if err != nil {
...
func (dp *ccPolicyProvider) getChannelContext() (context.ChannelProvider, error) {
	//Get Channel Context
