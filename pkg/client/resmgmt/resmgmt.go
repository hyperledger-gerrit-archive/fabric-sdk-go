...
//  options holds optional request options
//  if options have signatures (WithCfgSignatures() or 1 or more WithCfgSignaturesReader() calls), then SaveChannel will
//     use these signatures instead of creating signatures for the SigningIdentities found in req
...
// CreateCfgSignature creates a signature for the given client, custom signers and chConfig from channelConfigPath argument
//	return ConfigSignature will be signed internally by the SDK
func (rc *Client) CreateCfgSignature(signer msp.SigningIdentity, channelConfigPath string) (*common.ConfigSignature, error) {
...
// PrepCfgForExternalSigning will prepare a the SignatureHeader and the full signing []byte (signingBytes) to be used for signing a Channel Config
// 	Once signingBytes have been signed externally, create a ConfigSignature{} instance with 'signatureHeader' and the generated signature from 'signingBytes'
//  Then use WithCfgSignatures() option to pass this new instance for channel updates
func (rc *Client) PrepCfgForExternalSigning(signer msp.SigningIdentity, channelConfigPath string) (signatureHeader common.SignatureHeader, signingBytes []byte, e error) {
	chConfig, err := readChConfigData(channelConfigPath)
	if err != nil {
		e = err
		return
	...
	signatureHeader, _, signingBytes, e = resource.PrepareCfgForSigning(&sigCtx, chConfig)
	return
}

// MarshalCfgSignature marshals 1 ConfigSignature for the given client concatenated as []byte
func MarshalCfgSignature(signature *common.ConfigSignature) ([]byte, error) {
...
// UnMarshalCfgSignature reads 1 ConfigSignature as []byte from reader and unmarshals it
func UnMarshalCfgSignature(reader io.Reader) (*common.ConfigSignature, error) {
	arr, err := readCfgSigArray(reader)
	if err != nil {
...
func readCfgSigArray(reader io.Reader) ([]byte, error) {
	buff := make([]byte, 1024)
	arr := []byte{}
	for {
