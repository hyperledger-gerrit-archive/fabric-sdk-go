...
// CreateConfigSignatureSerialize will prepare a SignatureHeader and the full signing []byte (signingBytes) to be used for signing a Channel Config
// 	Once SigningBytes have been signed externally (signing signatureHeader.SigningBytes using an external tool like OpenSSL), do the following:
//  1. create a ConfigSignature{} instance
//  2. assign ConfigSignature.SignatureHeader with the returned field 'signatureHeaderData.signatureHeader'
//  3. assign ConfigSignature.Signature with the generated signature of 'signatureHeaderData.signingBytes' from the external tool
//  Then use WithConfigSignatures() option to pass this new instance for channel updates
func (rc *Client) CreateConfigSignatureSerialize(signer msp.SigningIdentity, channelConfigPath string) (signatureHeaderData resource.ConfigSignatureData, e error) {
	chConfig, err := readChConfigData(channelConfigPath)
	if err != nil {
		e = err
		return
	...
	return resource.GetConfigSerializedData(&sigCtx, chConfig)
}
