...
func (rc *Client) getConfigSignatures(req SaveChannelRequest, chConfig []byte) ([]*common.ConfigSignature, error) {
	...
	return getConfigSignaturesFromIdentities(rc, signers, chConfig)
}
...
// GetConfigSignaturesFromIdentitiesAndChConfigPath creates signatures for the given client, custom signers and chConfig from channelConfigPath argument
func GetConfigSignaturesFromIdentitiesAndChConfigPath(client *Client, signers []msp.SigningIdentity, channelConfigPath string) ([]*common.ConfigSignature, error) {
	...
	return getConfigSignaturesFromIdentities(client, signers, chConfig)
}

// GetConfigSignaturesFromIdentitiesAndChConfig creates signatures for the given client, custom signers and channelConfigReader argument
func GetConfigSignaturesFromIdentitiesAndChConfig(client *Client, signers []msp.SigningIdentity, channelConfigReader io.Reader) ([]*common.ConfigSignature, error) {
	chConfig, err := extractChConfigData(channelConfigReader)
	if err != nil {
		return nil, errors.WithMessage(err, "extracting channel config from channelConfigReader failed")
	}

	return getConfigSignaturesFromIdentities(client, signers, chConfig)
}

func getConfigSignaturesFromIdentities(client *Client, signers []msp.SigningIdentity, chConfig []byte) ([]*common.ConfigSignature, error) {
	var configSignatures []*common.ConfigSignature
	for _, signer := range signers {

		sigCtx := contextImpl.Client{
			SigningIdentity: signer,
			Providers:       client.ctx,
		}
