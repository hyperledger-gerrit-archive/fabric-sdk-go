...
type SaveChannelRequest struct {
	ChannelID           string
	ChannelConfigBlock  *common.Block         // Updated channel config block. If present, takes precedence over ChannelConfig
	ChannelConfigTx     io.Reader             // ChannelConfig Tx data source
...
func (rc *Client) SaveChannel(req SaveChannelRequest, options ...RequestOption) (SaveChannelResponse, error) {
	...
	if req.ChannelConfigTxPath != "" {
		...
		req.ChannelConfigTx = configReader
	}
...
func (rc *Client) prepareChConfigTx(req SaveChannelRequest, orderer fab.Orderer) ([]byte, error) {
	if req.ChannelConfigBlock != nil {
		b, err := rc.calculateChConfigTx(req, orderer)
		if err != nil {
			return nil, errors.Errorf("failed to calculate channel config transaction: %v", err)
		}
		reader, err := rc.createConfigEnvelopeReader(req.ChannelConfigBlock.Data.Data[0], b)
		if err != nil {
			return nil, errors.Errorf("failed to create config envelope reader: %v", err)
		}
		req.ChannelConfigTx = reader
	}
	return extractChConfigTx(req.ChannelConfigTx)
}

func (rc *Client) createConfigEnvelopeReader(blockData []byte, configUpdateBytes []byte) (io.Reader, error) {
...
func (rc *Client) calculateChConfigTx(req SaveChannelRequest, orderer fab.Orderer) ([]byte, error) {
	updatedConfig, err := extractConfigFromBlock(req.ChannelConfigBlock)
	if err != nil {
		return nil, errors.WithMessage(err, "extracting config from updated block failed")
	}
	...
	if block.Header.Number != req.ChannelConfigBlock.Header.Number {
		return nil, errors.New("block number mismatch")
	}
	currentConfig, err := extractConfigFromBlock(block)
	if err != nil {
...
func (rc *Client) validateSaveChannelRequest(req SaveChannelRequest) error {
	...
	if req.ChannelID == "" || (req.ChannelConfigBlock == nil && req.ChannelConfigTx == nil) {
