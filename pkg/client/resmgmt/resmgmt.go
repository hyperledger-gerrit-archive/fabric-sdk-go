...
// CreateConfigSignatureExternal will prepare a the SignatureHeader and the full signing []byte (signingBytes) to be used for signing a Channel Config
...
func (rc *Client) CreateConfigSignatureExternal(signer msp.SigningIdentity, channelConfigPath string) (signatureHeader common.SignatureHeader, signingBytes []byte, e error) {
	chConfig, err := readChConfigData(channelConfigPath)
	if err != nil {
		e = err
		return
