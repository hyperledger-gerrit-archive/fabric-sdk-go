...
type Opts struct {
	Targets      []context.Peer // target peers
...
type ResourceMgmtClient struct {
	...
	discoveryProvider context.DiscoveryProvider // used to get per channel discovery service(s)
	channelProvider   context.ChannelProvider
	fabricProvider    context.FabricProvider
	discovery         context.DiscoveryService // global discovery service (detects all peers on the network)
...
func (f *MSPFilter) Accept(peer context.Peer) bool {
...
type Context struct {
	...
	DiscoveryProvider context.DiscoveryProvider
	ChannelProvider   context.ChannelProvider
	FabricProvider    context.FabricProvider
	Resource          context.Resource
}
...
func (rc *ResourceMgmtClient) JoinChannel(channelID string, options ...Option) error {
	...
	joinChannelRequest := context.JoinChannelRequest{
...
func filterTargets(peers []context.Peer, filter TargetFilter) []context.Peer {
...
func (rc *ResourceMgmtClient) getDefaultTargets(discovery context.DiscoveryService) ([]context.Peer, error) {
...
func (rc *ResourceMgmtClient) calculateTargets(discovery context.DiscoveryService, peers []context.Peer, filter TargetFilter) ([]context.Peer, error) {
...
func (rc *ResourceMgmtClient) isChaincodeInstalled(req InstallCCRequest, peer context.Peer) (bool, error) {
...
func (rc *ResourceMgmtClient) InstallCC(req InstallCCRequest, options ...Option) ([]InstallCCResponse, error) {
	...
	newTargets := make([]context.Peer, 0)
	...
	icr := context.InstallChaincodeRequest{Name: req.Name, Path: req.Path, Version: req.Version, Package: req.Package, Targets: peer.PeersToTxnProcessors(newTargets)}
...
func (rc *ResourceMgmtClient) QueryInstalledChaincodes(proposalProcessor context.ProposalProcessor) (*pb.ChaincodeQueryResponse, error) {
...
func createAndSendTransaction(sender context.Sender, request context.TransactionRequest) (*context.TransactionResponse, error) {
