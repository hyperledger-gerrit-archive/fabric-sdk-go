...
func New(ctx core.Providers, opts ...ClientOption) (*Client, error) {
	...
	caClient, err := caclient.New(client.orgName, ctx.IdentityManager(), ctx.StateStore(), ctx.CryptoSuite(), ctx.Config())
	if err != nil {
		return nil, errors.WithMessage(err, "failed to create CA Client")
	}
...
func (c *Client) Register(request *caapi.RegistrationRequest) (string, error) {
	return c.client.Register(request)
}

// Revoke revokes a User with the Fabric CA
// request: Revocation Request
func (c *Client) Revoke(request *caapi.RevocationRequest) (*caapi.RevocationResponse, error) {
