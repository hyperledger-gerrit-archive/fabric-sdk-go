...
func Example() {
	...
	if err != nil {
		fmt.Printf("failed to query block: %v\n", err)
	}
...
func ExampleWithTargets() {
	...
	if err != nil {
		fmt.Printf("failed to query config with target peer: %v\n", err)
	}
...
func ExampleClient_QueryBlockByTxID() {
	...
	if err != nil {
		fmt.Printf("failed to query block by transaction ID: %v\n", err)
	}
...
func ExampleClient_QueryConfig() {
	...
	cfg, err := c.QueryConfig(WithTargets(mockPeerWithConfigBlock()))
	if err != nil {
		fmt.Printf("failed to query config: %v\n", err)
	}
