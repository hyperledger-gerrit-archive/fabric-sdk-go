...
func (c *Client) QueryInfo(options ...RequestOption) (*fab.BlockchainInfoResponse, error) {
	...
	responses, err := c.ledger.QueryInfo(reqCtx, peersToTxnProcessors(targets), c)
...
func (c *Client) QueryBlockByHash(blockHash []byte, options ...RequestOption) (*common.Block, error) {
	...
	responses, err := c.ledger.QueryBlockByHash(reqCtx, blockHash, peersToTxnProcessors(targets), c)
	if err != nil && len(responses) == 0 {
...
func (c *Client) QueryBlock(blockNumber uint64, options ...RequestOption) (*common.Block, error) {
	...
	responses, err := c.ledger.QueryBlock(reqCtx, blockNumber, peersToTxnProcessors(targets), c)
	if err != nil && len(responses) == 0 {
...
func (c *Client) QueryTransaction(transactionID fab.TransactionID, options ...RequestOption) (*pb.ProcessedTransaction, error) {
	...
	responses, err := c.ledger.QueryTransaction(reqCtx, transactionID, peersToTxnProcessors(targets), c)
	if err != nil && len(responses) == 0 {
...
func (c *Client) Verify(response *fab.TransactionProposalResponse) error {
	...
	err := c.membership.Validate(creatorID)
	if err != nil {
	...
	err = c.membership.Verify(creatorID, digest, res.GetEndorsement().Signature)
	if err != nil {
...
func (c *Client) Match(response []*fab.TransactionProposalResponse) error {
	return nil
}
