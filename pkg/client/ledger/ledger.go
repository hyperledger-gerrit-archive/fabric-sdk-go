...
func (c *Client) QueryInfo(options ...RequestOption) (*fab.BlockchainInfoResponse, error) {
	...
	reqCtx, cancel, targets, opts, err := c.prepareRequestParams(options...)
	if err != nil {
...
func (c *Client) prepareRequestParams(options ...RequestOption) (reqContext.Context, reqContext.CancelFunc, []fab.Peer, *requestOptions, error) {
	opts, err := c.prepareRequestOpts(options...)
	if err != nil {
		return nil, nil, nil, nil, errors.WithMessage(err, "failed to get opts")
	}

	targets, err := c.calculateTargets(opts)
	if err != nil {
		return nil, nil, nil, nil, errors.WithMessage(err, "failed to determine target peers")
	}

	reqCtx, cancel := c.createRequestContext(&opts)

	return reqCtx, cancel, targets, &opts, nil
}
