...
func New(clientProvider context.ClientProvider, chName string, opts ...ClientOption) (*Client, error) {
	...
	l, err := channel.NewLedger(clientContext, chName)
	if err != nil {
		return nil, err
	}

	discoveryService, err := clientContext.DiscoveryProvider().NewDiscoveryService(chName)
	if err != nil {
		return nil, err
	}
