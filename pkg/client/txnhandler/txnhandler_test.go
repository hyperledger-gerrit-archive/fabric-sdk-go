...
func TestQueryHandlerSuccess(t *testing.T) {
	...
	//Sample request
	request := api.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}}

	//Prepare context objects for handler
	requestContext := prepareRequestContext(request, api.Opts{}, t)
	...
	clientContext := setupChannelClientContext(nil, nil, []context.Peer{mockPeer1, mockPeer2}, t)
...
func TestExecuteTxHandlerSuccess(t *testing.T) {
	//Sample request
	request := api.Request{ChaincodeID: "test", Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}}
...
func TestEndorsementHandler(t *testing.T) {
	...
	requestContext := prepareRequestContext(request, api.Opts{ProposalProcessors: []context.ProposalProcessor{fcmocks.NewMockPeer("p2", "")}}, t)
...
func TestProposalProcessorHandler(t *testing.T) {
	...
	discoveryPeers := []context.Peer{peer1, peer2}
	...
	requestContext = prepareRequestContext(request, api.Opts{ProposalProcessors: []context.ProposalProcessor{peer2}}, t)
	handler.Handle(requestContext, setupChannelClientContext(nil, nil, discoveryPeers, t))
	if requestContext.Error != nil {
		t.Fatalf("Got error: %s", requestContext.Error)
	}
...
func prepareRequestContext(request api.Request, opts api.Opts, t *testing.T) *api.RequestContext {

	var requestContext *api.RequestContext

	requestContext = &api.RequestContext{Request: request,
		Opts:     opts,
...
func setupChannelClientContext(discErr error, selectionErr error, peers []context.Peer, t *testing.T) *api.ClientContext {
...
func setupTestSelection(discErr error, peers []context.Peer) (context.SelectionService, error) {
