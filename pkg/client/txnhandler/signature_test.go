...
func TestSignatureValidationHandlerSuccess(t *testing.T) {
	...
	clientContext := setupContextForSignatureValidation(fcmocks.NewMockMSPManager(msps), []context.Peer{mockPeer1}, t)
...
func TestSignatureValidationMspErrors(t *testing.T) {
	...
	msps := make(map[string]msp.MSP)
	clientContext := setupContextForSignatureValidation(fcmocks.NewMockMSPManagerWithError(msps, errors.New("GetMSPs")), []context.Peer{mockPeer1}, t)
	handler.Handle(requestContext, clientContext)
	verifyExpectedError(requestContext, "GetMSPs return error", t)

	// Test #2: MPS manager has no mps
	clientContext = setupContextForSignatureValidation(fcmocks.NewMockMSPManager(nil), []context.Peer{mockPeer1}, t)
	handler.Handle(requestContext, clientContext)
...
func verifyExpectedError(requestContext *api.RequestContext, expected string, t *testing.T) {
...
func setupContextForSignatureValidation(mspMgr *fcmocks.MockMSPManager, peers []context.Peer, t *testing.T) *api.ClientContext {
