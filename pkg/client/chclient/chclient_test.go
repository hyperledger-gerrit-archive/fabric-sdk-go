...
func TestTxProposalResponseFilter(t *testing.T) {
	...
	peers := []context.Peer{testPeer1, testPeer2}
	chClient := setupChannelClient(peers, t)

	_, err := chClient.Query(api.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err == nil {
...
func TestQuery(t *testing.T) {
	...
	_, err := chClient.Query(api.Request{})
	if err == nil {
		t.Fatalf("Should have failed for empty query request")
	}

	_, err = chClient.Query(api.Request{Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err == nil {
		t.Fatalf("Should have failed for empty chaincode ID")
	}

	_, err = chClient.Query(api.Request{ChaincodeID: "testCC", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err == nil {
		t.Fatalf("Should have failed for empty function")
	}

	response, err := chClient.Query(api.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err != nil {
	...
	chClient = setupChannelClient([]context.Peer{testPeer1, testPeer2}, t)
	_, err = chClient.Query(api.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
	if err == nil {
...
func TestQueryWithOptAsync(t *testing.T) {
	...
	go func() {
		resp, err := chClient.Query(api.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}})
...
func TestQueryWithOptTarget(t *testing.T) {
	...
	peers := []context.Peer{testPeer}

	targets := peer.PeersToTxnProcessors(peers)

	response, err := chClient.Query(api.Request{ChaincodeID: "testCC", Fcn: "invoke",
...
func TestExecuteTx(t *testing.T) {
	...
	_, err := chClient.Execute(api.Request{})
	if err == nil {
		t.Fatalf("Should have failed for empty invoke request")
	}

	_, err = chClient.Execute(api.Request{Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	if err == nil {
		t.Fatalf("Should have failed for empty chaincode ID")
	}

	_, err = chClient.Execute(api.Request{ChaincodeID: "testCC", Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	if err == nil {
		t.Fatalf("Should have failed for empty function")
	}
	...
	testPeer2.Payload = []byte("test2")
	chClient = setupChannelClient([]context.Peer{testPeer1, testPeer2}, t)
	_, err = chClient.Execute(api.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("move"), []byte("b")}})
	if err == nil {
		t.Fatalf("Should have failed")
	}
	s, ok := status.FromError(err)
...
func (c *customHandler) Handle(requestContext *api.RequestContext, clientContext *api.ClientContext) {
...
func TestInvokeHandler(t *testing.T) {
	...
	response, err := chClient.InvokeHandler(handler, api.Request{ChaincodeID: "testCC", Fcn: "move", Args: [][]byte{[]byte("a"), []byte("b"), []byte("1")}})
	if err != nil {
...
func (h *customEndorsementHandler) Handle(requestContext *api.RequestContext, clientContext *api.ClientContext) {
...
func TestQueryWithCustomEndorser(t *testing.T) {
	...
	response, err := chClient.InvokeHandler(
		...
		api.Request{ChaincodeID: "testCC", Fcn: "invoke", Args: [][]byte{[]byte("query"), []byte("b")}},
	)
	if err != nil {
		t.Fatalf("Failed to invoke test cc: %s", err)
	}
...
func TestExecuteTxDiscoveryError(t *testing.T) {
	...
	_, err := chClient.Execute(api.Request{ChaincodeID: "testCC", Fcn: "invoke",
...
func TestRPCStatusErrorPropagation(t *testing.T) {
	...
	chClient := setupChannelClient([]context.Peer{testPeer1}, t)
...
func TestOrdererStatusError(t *testing.T) {
	...
	testPeer1 := fcmocks.NewMockPeer("Peer1", "http://peer1.com")
	peers := []context.Peer{testPeer1}
	testOrderer1 := fcmocks.NewMockOrderer("", make(chan *context.SignedEnvelope))
	orderers := []context.Orderer{testOrderer1}
	...
	_, err := chClient.Execute(api.Request{ChaincodeID: "test", Fcn: "invoke",
		Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
	statusError, ok := status.FromError(err)
...
func TestTransactionValidationError(t *testing.T) {
	...
	response, err := chClient.Execute(api.Request{ChaincodeID: "test", Fcn: "invoke",
		Args: [][]byte{[]byte("move"), []byte("a"), []byte("b"), []byte("1")}})
...
func TestDiscoveryGreylist(t *testing.T) {
	...
	testChannelSvc, err := setupTestChannelService(fabCtx, []context.Orderer{orderer})
	assert.Nil(t, err, "Got error %s", err)

	discoveryService, err := setupTestDiscovery(nil, []context.Peer{testPeer1})
	assert.Nil(t, err, "Got error %s", err)
...
func setupTestChannelService(ctx context.Context, orderers []context.Orderer) (context.ChannelService, error) {
...
func setupTestDiscovery(discErr error, peers []context.Peer) (context.DiscoveryService, error) {
...
func setupTestSelection(discErr error, peers []context.Peer) (*txnmocks.MockSelectionService, error) {
...
func setupChannelClient(peers []context.Peer, t *testing.T) *ChannelClient {

	return setupChannelClientWithError(nil, nil, peers, t)
}

func setupChannelClientWithError(discErr error, selectionErr error, peers []context.Peer, t *testing.T) *ChannelClient {
...
func setupChannelClientWithNodes(peers []context.Peer,
	orderers []context.Orderer, t *testing.T) *ChannelClient {
...
func createAndSendTransactionProposal(sender context.ProposalSender, chrequest *api.Request, targets []context.ProposalProcessor) ([]*context.TransactionProposalResponse, context.TransactionID, error) {
	...
	if err != nil {
		return nil, context.TransactionID{}, errors.WithMessage(err, "creation of transaction proposal failed")
	}
