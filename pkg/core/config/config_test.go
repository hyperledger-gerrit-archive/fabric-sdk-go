...
func TestConfigBackend_Lookup(t *testing.T) {
	...
	checkConfigStringKey(t, "name", "global-trade-network", true, err)

	checkConfigStringKey(t, "description", "", false, err)

	checkConfigStringKey(t, "x-type", "h1fv1", true, err)

	checkConfigMapKey(t, "channels.mychannel.peers", 1, err)

	checkConfigMapKey(t, "organizations", 3, err)
}

func checkConfigStringKey(t *testing.T, configKey string, expectedValue string, checkNotEquals bool, configGeneralError error) {
	value, ok := configBackend.Lookup(configKey)
	if !ok {
		t.Fatal(configGeneralError)
	}
	v := value.(string)
...
func checkConfigMapKey(t *testing.T, configKey string, expectedNumItems int, configGeneralError error) {
	value, ok := configBackend.Lookup(configKey)
	if !ok {
		t.Fatal(configGeneralError)
	}
