...
func TestConfig_Value(t *testing.T) {
	...
	value, err := configImpl.Value("name")
	if err != nil {
		t.Fatal(err)
	}
	name := value.(string)
	...
	value, err = configImpl.Value("description")
	if err != nil {
		t.Fatal(err)
	}
	...
	value, err = configImpl.Value("x-type")
	if err != nil {
		t.Fatal(err)
	}
	xType := value.(string)
	...
	value, err = configImpl.Value("channels.mychannel.chaincodes")
	if err != nil {
		t.Fatal(err)
	}
