...
func TestTLSCAConfigWithMultipleCerts(t *testing.T) {
	...
	assert.Nil(t, err)
	verifyCertPoolInstance(t, pool, tlsCertPool, 0, 0, numberOfSubjects)

	//add 2 certs
	pool, err = tlsCertPool.Get(certOrderer, certOrg1)
	assert.Nil(t, err)
	verifyCertPoolInstance(t, pool, tlsCertPool, 2, 2, numberOfSubjects)

	//add 1 existing cert
	pool, err = tlsCertPool.Get(certOrg1)
	assert.Nil(t, err)
	verifyCertPoolInstance(t, pool, tlsCertPool, 2, 2, numberOfSubjects)
	...
	//add 2 existing certs
	pool, err = tlsCertPool.Get(certOrderer, certOrg1)
	assert.Nil(t, err)
	verifyCertPoolInstance(t, pool, tlsCertPool, 2, 2, numberOfSubjects)

	//add 3 certs, (2 existing + 1 new)
	pool, err = tlsCertPool.Get(certOrderer, certOrg1, certOrg2)
	assert.Nil(t, err)
	verifyCertPoolInstance(t, pool, tlsCertPool, 3, 3, numberOfSubjects)

	//add all 3 existing certs
	pool, err = tlsCertPool.Get(certOrderer, certOrg1, certOrg2)
	assert.Nil(t, err)
	verifyCertPoolInstance(t, pool, tlsCertPool, 3, 3, numberOfSubjects)
...
func verifyCertPoolInstance(t *testing.T, pool *x509.CertPool, fabPool fab.CertPool, numberOfCerts, numberOfCertsByName, numberOfSubjects int) {
	...
	assert.Equal(t, numberOfSubjects+numberOfCerts, len(tlsCertPool.certPool.Subjects()))
	assert.Equal(t, numberOfSubjects+numberOfCerts, len(pool.Subjects()))
}
...
func TestAddingDuplicateCertsToPool(t *testing.T) {
	...
	pool, err = tlsCertPool.Get(certOrderer, certOrg1, certOrderer, certOrg1, certOrg1, certOrg1, certOrderer, certOrderer)
	assert.Nil(t, err)
	verifyCertPoolInstance(t, pool, tlsCertPool, 2, 2, numberOfSubjects)

	//add multiple certs with duplicate
	pool, err = tlsCertPool.Get(certOrderer, certOrg1, certOrderer, certOrg1, certOrg2, certOrg2, certOrg2, certOrderer, certOrderer)
	assert.Nil(t, err)
	verifyCertPoolInstance(t, pool, tlsCertPool, 3, 3, numberOfSubjects)
}
...
func TestRemoveDuplicatesCerts(t *testing.T) {
	...
	certs := tlsCertPool.removeDuplicates(certOrg1, certOrg2, certOrg1, certOrg1, certOrg1, certOrg1, certOrderer)
