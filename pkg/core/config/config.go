...
func (c *Config) tryMatchingCAConfig(caName string) (*core.CAConfig, string, error) {
	...
	return nil, "", errors.WithStack(status.New(status.ClientStatus, status.NoMatchingCertificateAuthority.ToInt32(), "no matching certAuthority config found", nil))
}
...
func (c *Config) tryMatchingPeerConfig(peerName string) (*core.PeerConfig, error) {
	...
	return nil, errors.WithStack(status.New(status.ClientStatus, status.NoMatchingPeerConfig.ToInt32(), "no matching peer config found", nil))
}
...
func (c *Config) tryMatchingOrdererConfig(ordererName string) (*core.OrdererConfig, error) {
	...
	return nil, errors.WithStack(status.New(status.ClientStatus, status.NoMatchingOrdererConfig.ToInt32(), "no matching orderer config found", nil))
}
...
func (c *Config) findMatchingPeer(peerName string) (string, error) {
	...
	return "", errors.WithStack(status.New(status.ClientStatus, status.NoMatchingPeerConfig.ToInt32(), "no matching peer config found", nil))
}
