...
func (handle *ContextHandle) GetSession() (session mPkcs11.SessionHandle) {
	...
	default:
		logger.Debug("Sept5.5 opening a new session since cache is empty/full")
		...
		if err != nil {
			...
			for i := 0; i < handle.opts.openSessionRetryAttempts; i++ {
				logger.Debugf(" **** sleeping for [%v] before trying attempt[%d]", handle.opts.openSessionRetryInterval, i+1)
				//try to relogin
				time.Sleep(handle.opts.openSessionRetryInterval)
				logger.Debugf(" **** attempt[%d]", i+1)

				logger.Debugf("Trying to re-login, attempt[%d], for error[%v]", i+1, err)
				newSession, err := handle.softReLogin()
				logger.Debugf("Re-login attempted, attempt[%d], will try to open session again", i+1)

				if err != nil {
					logger.Debugf("Re-login failed, attempt[%d], trying again, err [%s]", i+1, err)
					continue
				} else {
					logger.Debugf("Re-login succeeded, attempt[%d],returning session [%d]", i+1, newSession)
					session = newSession
			...
			logger.Debugf("** Failed to open session, exhausted all %d attempts, giving up now", handle.opts.openSessionRetryAttempts)
