...
func (handle *ContextHandle) GetSession() (session mPkcs11.SessionHandle) {
	id := time.Now().Unix()
	logger.Debug("Sept5.12 getting read lock for , id[%d]", id)
	handle.lock.RLock()
	logger.Debug("Sept5.12 got read lock for , id[%d]", id)
	...
	default:
		logger.Debug("Sept5.12 opening a new session since cache is empty/full, id[%d]", id)
		handle.lock.RUnlock()

		logger.Debug("Sept5.12 -- trying to open session, id[%d]", id)
		// cache is empty (or completely in use), create a new session
		s, err := handle.OpenSession()
		logger.Debug("Sept5.12 -- attempted to open session, id[%d], session[%d], err[%s]", id, session, err)
			...
			for i := 0; i < handle.opts.openSessionRetryAttempts; i++ {
				...
				if err != nil {
					...
					if i < handle.opts.openSessionRetryAttempts-1 {
						...
						logger.Debugf("**5.12** OpenSession failed [%s] : [%d] : returning session[%d]", err, id, session)
						return 0
					}
				} else {
