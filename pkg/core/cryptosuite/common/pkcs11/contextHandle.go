...
func (handle *ContextHandle) GetSession() (session mPkcs11.SessionHandle) {
	id := time.Now()
	logger.Debug("Sept5.10 getting read lock for , id[%v]", id)
	handle.lock.RLock()
	logger.Debug("Sept5.10 got read lock for , id[%v]", id)
	...
	default:
		logger.Debug("Sept5.10 opening a new session since cache is empty/full, id[%v]", id)
			...
			for i := 0; i < handle.opts.openSessionRetryAttempts; i++ {
				...
				if err != nil {
					...
					if i < handle.opts.openSessionRetryAttempts-1 {
						...
						logger.Debugf("**5.10** OpenSession failed [%s] : [%v] : returning session[%d]", err, id, session)
						return 0
					}
				} else {
