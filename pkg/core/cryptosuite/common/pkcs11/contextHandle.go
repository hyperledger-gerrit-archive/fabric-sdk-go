...
func (handle *ContextHandle) GetSession() (session mPkcs11.SessionHandle) {
	...
	default:
		logger.Debug("Sept5.4 opening a new session since cache is empty/full")
		for i := 0; i < handle.opts.openSessionRetryAttempts; i++ {
			logger.Debugf("Trying to open session, attempt[%d]", i+1)
			// cache is empty (or completely in use), create a new session
			s, err := handle.OpenSession()
			if err != nil {
				logger.Debugf("Failed to open session, attempt[%d], error[%v]", i+1, err)
				//try to relogin
			...
			logger.Debugf("Created new pkcs11 session %+v on slot %d, attempt [%d]", s, handle.slot, i+1)
			session = s
			logger.Debugf("Clearing cache for new session [%d], attempt [%d]", s, i+1)
			cachebridge.ClearSession(fmt.Sprintf("%d", session))
		}
	}
	handle.lock.RUnlock()
