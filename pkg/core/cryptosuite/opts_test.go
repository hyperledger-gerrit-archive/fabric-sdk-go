...
func TestCreateCustomFullCryptotConfig(t *testing.T) {
	...
	cryptoConfigOption, err := BuildConfigCryptoFromOptions(opts...)
	if err != nil {
		t.Fatalf("BuildConfigCryptoFromOptions returned unexpected error %s", err)
	}
	if cryptoConfigOption == nil {
		t.Fatalf("BuildConfigCryptoFromOptions call returned nil")
	}
}

func TestCreateCustomCryptoConfig(t *testing.T) {
	// try to build with partial implementations
	cryptoConfigOption, err := BuildConfigCryptoFromOptions(m1, m2, m3, m4)
	if err != nil {
		t.Fatalf("BuildConfigCryptoFromOptions returned unexpected error %s", err)
	}
	...
	if cco, ok = cryptoConfigOption.(*CryptoConfigOptions); !ok {
		t.Fatalf("BuildConfigCryptoFromOptions did not return an Options instance %T", cryptoConfigOption)
	}
	...
	// verify if an interface was not passed as an option but was not nil, it should be nil (ie these implementations should not populated in cco: m5, m6, m7, m8 and m9)
...
func TestCreateCustomCryptoConfigRemainingFunctions(t *testing.T) {
	...
	cryptoConfigOption, err := BuildConfigCryptoFromOptions(m5, m6, m7, m8, m9)
	if err != nil {
		t.Fatalf("BuildConfigCryptoFromOptions returned unexpected error %s", err)
	}
	var cco *CryptoConfigOptions
	...
	// verify if an interface was not passed as an option but was not nil, it should be nil (ie these implementations should not populated in cco: m1, m2, m3 and m4)
	...
	_, err = BuildConfigCryptoFromOptions(m4, m5, m7, badType)
	require.Error(t, err, "BuildConfigCryptoFromOptions did not return error with badType")
...
func TestCreateCustomCryptoConfigWithSomeDefaultFunctions(t *testing.T) {
	...
	// ie, verify these implementations should nil: m5, m6, m7, m8 and m9
	...
	// ie, verify these implementations should now populated in cco: m5, m6, m7, m8 and m9
