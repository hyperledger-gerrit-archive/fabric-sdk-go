...
func TestCAConfigSecurityAlgorithmCase(t *testing.T) {

	// we expect the following values
	const expectedAlgVal = "sha256"

	// map key represents what we will input
	providerTestValues := map[string]string{
		// all upper case
		"SHA256": expectedAlgVal,
		// all lower case
		"sha256": expectedAlgVal,
		// mixed case
		"Sha256": expectedAlgVal,
	...
	for inputValue, expectedValue := range providerTestValues {
		...
		// set the input value, overriding what's in file
		os.Setenv("FABRIC_SDK_CLIENT_BCCSP_SECURITY_HASHALGORITHM", inputValue)
		...
		// expected values should be uppercase
		if expectedValue != cryptoConfig.SecurityAlgorithm() {
			t.Fatalf(
				"Incorrect BCCSP SecurityAlgorithm - input:%s actual:%s, expected:%s",
				inputValue,
				cryptoConfig.SecurityProvider(),
				expectedValue,
			)
