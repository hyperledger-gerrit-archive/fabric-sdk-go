...
func TestStore(t *testing.T) {
	...
	if err := store.Delete(userKey(user1)); err != nil {
	...
	if err := store.Delete(userKey(user2)); err != nil {
...
func checkStoreValue(store *CertFileUserStore, user api.UserData, expected []byte) error {
	userKey := userKey(user)
	storeKey := storeKeyFromUserKey(userKey)
...
func userKey(userData api.UserData) api.UserKey {
	return api.UserKey{MspID: userData.MspID, Name: userData.Name}
}
