...
func userKeyFromUser(user contextApi.UserData) contextApi.UserKey {
...
func (s *CertFileUserStore) Load(key contextApi.UserKey) (contextApi.UserData, error) {
...
func (s *CertFileUserStore) Store(user contextApi.UserData) error {
	key := storeKeyFromUserKey(contextApi.UserKey{MspID: user.MspID, Name: user.Name})
	return s.store.Store(key, user.EnrollmentCertificate)
}

// Delete deletes a User from store
func (s *CertFileUserStore) Delete(key contextApi.UserKey) error {
	return s.store.Delete(storeKeyFromUserKey(key))
}
