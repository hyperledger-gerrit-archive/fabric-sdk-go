...
func newUser(userData api.UserData, cryptoSuite core.CryptoSuite) (*user, error) {
...
func (mgr *IdentityManager) newUser(userData api.UserData) (*user, error) {
...
func (mgr *IdentityManager) loadUserFromStore(userName string) (api.User, error) {
	...
	var user api.User
	userData, err := mgr.userStore.Load(api.UserIdentifier{MspID: mgr.orgMspID, Name: userName})
	if err != nil {
		return nil, err
	}
