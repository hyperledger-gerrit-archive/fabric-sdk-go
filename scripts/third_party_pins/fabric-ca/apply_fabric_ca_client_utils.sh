...
sed -i'' -e 's/_, _, _, err := util.GetSignerFromCertFile(c.certFile, c.csp)/ certBytes, err := util.ReadFile(c.certFile)  \
if err != nil { return err } \
_, _, _, err = util.GetSignerFromCertFile(certBytes, c.csp) /g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
...
FILTER_FN=",NewCredential,Type,Val,EnrollmentID,SetVal,Load,Store,CreateOAuthToken,RevokeSelf,getCSP"
gofilter
...
sed -i'' -e 's/GetSignerFromCertFile(certFile string, csp core.CryptoSuite)/GetSignerFromCertFile(certBytes []byte, csp core.CryptoSuite)/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
START_LINE=`grep -n "// Load cert file" "${TMP_PROJECT_PATH}/${FILTER_FILENAME}" | head -n 1 | awk -F':' '{print $1}'`
for i in {1..5}
do
    sed -i'' -e ${START_LINE}'d' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
done
