...
func launchChaincode(ccParams *chaincodeParams, tlsPath string, done chan struct{}) {
	...
	for {
		logVerbosef("Starting chaincode [%s, %s]", ccParams.chaincodeBinary(), ccParams.ccID)
		...
		select {
		case <-done:
			logVerbosef("Stopping chaincode [%s, %s, %d]", ccParams.chaincodeBinary(), ccParams.ccID, cmd.Process.Pid)
...
func (d *chaincoded) handleUploadToContainerRequest(w http.ResponseWriter, r *http.Request, containerName string) {
	...
	logVerbosef("Handling upload to container request [%s]", containerName)
...
func (d *chaincoded) handleStartContainerRequest(w http.ResponseWriter, r *http.Request, containerName string) {
	logVerbosef("Handling start container request [%s]", containerName)
	w.WriteHeader(204)
}
...
func runHTTPServer(addr string, dh *chaincoded, wg *sync.WaitGroup, done chan struct{}) {
	...
	go func() {
		...
		logVerbosef("HTTP server stopped [%s]", addr)
	}()
