...
crypto-gen:
	...
	@$(DOCKER_CMD) run -i \
		-v /$(abspath .):/opt/workspace/$(PACKAGE_NAME) -u $(shell id -u):$(shell id -g) \
		$(FABRIC_TOOLS_IMAGE):$(FABRIC_TOOLS_TAG) \
		//bin/bash -c "FABRIC_VERSION_DIR=fabric/$(FABRIC_CRYPTOCONFIG_VER) /opt/workspace/${PACKAGE_NAME}/test/scripts/generate_crypto.sh"
...
channel-config-gen:
	...
	@$(DOCKER_CMD) run -i \
		-v /$(abspath .):/opt/workspace/$(PACKAGE_NAME) -u $(shell id -u):$(shell id -g) \
		$(FABRIC_TOOLS_IMAGE):$(FABRIC_TOOLS_TAG) \
		//bin/bash -c "FABRIC_VERSION_DIR=fabric/$(FABRIC_CODELEVEL_VER)/ /opt/workspace/${PACKAGE_NAME}/test/scripts/generate_channeltx.sh"
...
channel-config-stable-gen:
	...
	@$(DOCKER_CMD) run -i \
		...
		//bin/bash -c "FABRIC_VERSION_DIR=fabric/$(FABRIC_STABLE_CODELEVEL_VER)/ /opt/workspace/${PACKAGE_NAME}/test/scripts/generate_channeltx.sh"
...
channel-config-prev-gen:
	...
	$(DOCKER_CMD) run -i \
		...
		//bin/bash -c "FABRIC_VERSION_DIR=fabric/$(FABRIC_PREV_CODELEVEL_VER)/ /opt/workspace/${PACKAGE_NAME}/test/scripts/generate_channeltx.sh"
...
ifneq ($(FABRIC_PRERELEASE_VERSION),)
	...
	$(DOCKER_CMD) run -i \
		...
		//bin/bash -c "FABRIC_VERSION_DIR=fabric/$(FABRIC_PRERELEASE_CODELEVEL_VER)/ /opt/workspace/${PACKAGE_NAME}/test/scripts/generate_channeltx.sh"
endif
...
ifeq ($(ARCH),x86_64)
	...
	@$(FABRIC_DEV_REGISTRY_PRE_CMD) && \
		$(DOCKER_CMD) run -i \
			...
			//bin/bash -c "FABRIC_VERSION_DIR=fabric/$(FABRIC_DEVSTABLE_CODELEVEL_VER)/ /opt/workspace/${PACKAGE_NAME}/test/scripts/generate_channeltx.sh"
endif
