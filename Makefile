...
FABRIC_STABLE_LOCAL_INTTEST      := true
#TODO commented for time being because of known issue
FABRIC_DEVSTABLE_LOCAL_INTTEST   := false
endif
...
ifeq ($(FABRIC_STABLE_LOCAL_INTTEST),true)
	@$(MAKE) -f $(MAKEFILE_THIS) clean
	@FABRIC_SDKGO_SUBTARGET=true $(MAKE) -f $(MAKEFILE_THIS) integration-tests-local-stable
endif
ifeq ($(FABRIC_DEVSTABLE_LOCAL_INTTEST),true)
	@$(MAKE) -f $(MAKEFILE_THIS) clean
	@FABRIC_SDKGO_SUBTARGET=true $(MAKE) -f $(MAKEFILE_THIS) integration-tests-local-devstable
endif
	@$(MAKE) -f $(MAKEFILE_THIS) clean
...
.PHONY: integration-tests-local-stable
integration-tests-local-stable: temp-clean depend populate
	@cd $(FIXTURE_DOCKERENV_PATH) && \
...
.PHONY: integration-tests-local-devstable
integration-tests-local-devstable: temp-clean depend populate
	@. $(FIXTURE_DOCKERENV_PATH)/devstable-env.sh && \
		$(FABRIC_DEV_REGISTRY_PRE_CMD) && \
		cd $(FIXTURE_DOCKERENV_PATH) && \
