...
integration-tests-stable: clean-tests depend populate
...
integration-tests-prev: clean-tests depend populate
...
integration-tests-prerelease: clean-tests depend populate
...
integration-tests-devstable: clean-tests depend populate
...
integration-tests-stable-revoked: clean-tests depend populate
	@cd $(FIXTURE_DOCKERENV_PATH) && \
...
integration-tests-stable-orderer-cert-expired: clean-tests depend populate
	@cd $(FIXTURE_DOCKERENV_PATH) && \
...
integration-tests-stable-peer-cert-expired: clean-tests depend populate
	@cd $(FIXTURE_DOCKERENV_PATH) && \
...
integration-tests-stable-pkcs11: clean-tests depend populate build-softhsm2-image
...
integration-tests-devstable-nomutualtls: clean-tests depend populate
	@. $(FIXTURE_DOCKERENV_PATH)/devstable-env.sh && \
		. $(FIXTURE_DOCKERENV_PATH)/nomutualtls-env.sh && \
		$(FABRIC_DEV_REGISTRY_PRE_CMD) && \
		cd $(FIXTURE_DOCKERENV_PATH) && \
		TEST_CHANGED_ONLY=true FABRIC_SDKGO_CODELEVEL_VER=$(FABRIC_DEVSTABLE_CODELEVEL_VER) FABRIC_SDKGO_CODELEVEL_TAG=$(FABRIC_DEVSTABLE_CODELEVEL_TAG) FABRIC_DOCKER_REGISTRY=$(FABRIC_DEV_REGISTRY) $(DOCKER_COMPOSE_CMD) -f docker-compose.yaml -f docker-compose-nopkcs11-test.yaml up --force-recreate --abort-on-container-exit
	@cd $(FIXTURE_DOCKERENV_PATH) && FABRIC_DOCKER_REGISTRY=$(FABRIC_DEV_REGISTRY) $(FIXTURE_SCRIPTS_PATH)/check_status.sh "-f ./docker-compose.yaml -f ./docker-compose-nopkcs11-test.yaml"
...
integration-test: clean-tests depend populate
...
integration-tests-local: clean-temp depend populate
...
integration-tests-stable-local: clean-temp depend populate
	@cd $(FIXTURE_DOCKERENV_PATH) && \
...
integration-tests-devstable-local: clean-temp depend populate
	@. $(FIXTURE_DOCKERENV_PATH)/devstable-env.sh && \
		$(FABRIC_DEV_REGISTRY_PRE_CMD) && \
		cd $(FIXTURE_DOCKERENV_PATH) && \
...
.PHONY: populate
populate: populate-vendor-noforce
...
.PHONY: clean
clean: clean-temp clean-fixtures clean-cache populate-clean
