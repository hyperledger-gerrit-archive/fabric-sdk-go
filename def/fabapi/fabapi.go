...
func NewSDK(options Options) (*FabricSDK, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to initialize state store")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to initialize signing manager")
	}
...
func (sdk *FabricSDK) NewChannelClient(channelName string, userName string) (apitxn.ChannelClient, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "unable to retrieve client from network config")
	}
...
func (sdk *FabricSDK) NewChannelClientWithOpts(channelName string, userName string, opt *ChannelClientOpts) (apitxn.ChannelClient, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to get pre-enrolled user session")
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to created new channel client")
	}
...
func (sdk *FabricSDK) NewPreEnrolledUser(orgID string, userName string) (apifabclient.User, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to get credential manager")
	}

	signingIdentity, err := credentialMgr.GetSigningIdentity(userName)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get signing identity")
	}

	user, err := NewPreEnrolledUser(sdk.ConfigProvider(), userName, signingIdentity)
	if err != nil {
		return nil, errors.Wrap(err, "NewPreEnrolledUser returned error")
	}
...
func (sdk *FabricSDK) NewPreEnrolledUserSession(orgID string, userName string) (*Session, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to get context for org")
	}

	user, err := sdk.NewPreEnrolledUser(orgID, userName)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get pre-enrolled user")
	}

	session, err := sdk.NewSession(context, user)
	if err != nil {
		return nil, errors.Wrap(err, "NewSession returned error")
	}
