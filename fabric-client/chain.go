...
var logger = logging.MustGetLogger("fabric_sdk_go_chain")
...
func NewChain(name string, client Client) (Chain, error) {
	//logging.SetLevel(logging.DEBUG, "fabric_sdk_go_chain")
	if name == "" {
...
func (c *chain) GetGenesisBlock(request *GenesisBlockRequest) (*common.Block, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Error biulding channel header: %v", err)
	}
	seekHeader, err := BuildHeader(creator, seekInfoHeader, request.Nonce)
	if err != nil {
		return nil, fmt.Errorf("Error biulding header: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Error sending enveloper: %v", err)
	}
	return block, nil
}
