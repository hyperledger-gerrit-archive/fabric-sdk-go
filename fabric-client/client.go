...
func (c *client) QueryChannels(peer Peer) (*pb.ChannelQueryResponse, error) {
	...
	signedProposal, err := CreateTransactionProposal("cscc", "", []string{"GetChannels"}, true, nil, c)
	if err != nil {
		return nil, fmt.Errorf("CreateTransactionProposal return error: %v", err)
	}

	transactionProposalResponses, err := SendTransactionProposal(signedProposal, 0, []Peer{peer})
	if err != nil {
...
func (c *client) QueryInstalledChaincodes(peer Peer) (*pb.ChaincodeQueryResponse, error) {
	...
	signedProposal, err := CreateTransactionProposal("lccc", "", []string{"getinstalledchaincodes"}, true, nil, c)
	if err != nil {
		return nil, fmt.Errorf("CreateTransactionProposal return error: %v", err)
	}
