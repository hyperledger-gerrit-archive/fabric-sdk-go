...
func (ec *eventsClient) Start() error {
	...
	if err != nil {
		return fmt.Errorf("Could n7053ot create client conn to %s (%v)", ec.peerAddress, err)
	}
	ec.clientConn = conn
...
func (ec *eventsClient) Stop() error {
	...
	if err != nil {
		logger.Debugf("Stream should be closed and client connection should be dropped %v", err)
	}
