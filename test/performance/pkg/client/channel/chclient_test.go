...
var sourceURL = fmt.Sprintf("%s:%d", testhost, testport)
...
func setUp(m *testing.M) {
	...
	mockEndorserServer.SetMockPeer(&MockPeer{MockName: "Peer1", MockURL: sourceURL, MockRoles: []string{}, MockCert: nil, MockMSP: "Org1MSP", Status: 200,
		Payload: payloadMap})

	// create a delivery channel for the orderer and the deliveryservice
	d := make(chan *pb.FilteredBlock, 10)

	fmt.Println("***************** Mocked Peer Started: ", mockEndorserServer.Start(sourceURL, d), " ******************************")
