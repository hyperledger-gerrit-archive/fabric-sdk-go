...
func testFailedTx(t *testing.T, testSetup BaseSetupImpl) {
	...
	tpResponses1, tx1, err := testSetup.CreateAndSendTransactionProposal(testSetup.Channel, testSetup.ChainCodeID, testSetup.ChannelID, args, []txnapi.TxnProposalProcessor{testSetup.Channel.PrimaryPeer()}, nil)
	if err != nil {
		t.Fatalf("CreateAndSendTransactionProposal return error: %v", err)
	}

	tpResponses2, tx2, err := testSetup.CreateAndSendTransactionProposal(testSetup.Channel, testSetup.ChainCodeID, testSetup.ChannelID, args, []txnapi.TxnProposalProcessor{testSetup.Channel.PrimaryPeer()}, nil)
	if err != nil {
		t.Fatalf("CreateAndSendTransactionProposal return error: %v", err)
	}
...
func testMultipleBlockEventCallbacks(t *testing.T, testSetup BaseSetupImpl) {
	...
	tpResponses, tx, err := testSetup.CreateAndSendTransactionProposal(testSetup.Channel, testSetup.ChainCodeID, testSetup.ChannelID, args, []txnapi.TxnProposalProcessor{testSetup.Channel.PrimaryPeer()}, nil)
	if err != nil {
