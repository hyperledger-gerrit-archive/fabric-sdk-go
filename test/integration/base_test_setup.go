...
func (setup *BaseSetupImpl) Initialize() error {
	...
	if err = fabricTxn.CreateChannel(client, ordererAdmin, org1Admin, channel, setup.ChannelConfig); err != nil {
		return fmt.Errorf("CreateChannel returned error: %v", err)
	}
	time.Sleep(time.Second * 3)
	if err = fabricTxn.JoinChannel(client, org1Admin, channel); err != nil {
...
func (setup *BaseSetupImpl) InstantiateCC(chainCodeID string, channelID string, chainCodePath string, chainCodeVersion string, args []string) error {
	...
	if err := fabricTxn.SendInstantiateCC(setup.Channel, chainCodeID, channelID, args, chainCodePath, chainCodeVersion, []api.Peer{setup.Channel.GetPrimaryPeer()}, setup.EventHub); err != nil {
		return err
	}
	return nil
}
...
func (setup *BaseSetupImpl) InstallCC(chainCodeID string, chainCodePath string, chainCodeVersion string, chaincodePackage []byte) error {
	...
	if err := fabricTxn.SendInstallCC(setup.Client, setup.Channel, chainCodeID, chainCodePath, chainCodeVersion, chaincodePackage, setup.Channel.GetPeers(), setup.GetDeployPath()); err != nil {
