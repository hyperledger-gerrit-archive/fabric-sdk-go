...
func (setup *BaseSetupImpl) InstallCC(chainCodeID string, chainCodePath string, chainCodeVersion string, chaincodePackage []byte) error {
	if err := fcUtil.SendInstallCC(setup.Chain, chainCodeID, chainCodePath, chainCodeVersion, chaincodePackage, setup.Chain.GetPeers()); err != nil {
...
func (setup *BaseSetupImpl) Initialize() error {
	...
	client, err := fcUtil.GetClient("admin", "adminpw")
	if err != nil {
