...
import (
	...
	"github.com/hyperledger/fabric-sdk-go/def/fabsdk"
...
func (setup *BaseSetupImpl) Initialize() error {
	...
	mspClient, err := fabsdk.NewCAClient(configImpl, setup.OrgID)
	if err != nil {
		return fmt.Errorf("Failed to get default msp client: %v", err)
	}

	client, err := fabsdk.NewClientWithUser("admin", "adminpw", setup.OrgID, "/tmp/enroll_user", configImpl, mspClient)
	if err != nil {
...
func (setup *BaseSetupImpl) GetChannel(client api.FabricClient, channelID string, orgs []string) (api.Channel, error) {
	...
	for _, org := range orgs {
		...
		for _, p := range peerConfig {
			endorser, err := fabsdk.NewPeer(fmt.Sprintf("%s:%d", p.Host, p.Port),
