...
func (setup *BaseSetupImpl) Initialize() error {
	...
	sdk, err := deffab.NewSDK(sdkSetup)
	if err != nil {
		return fmt.Errorf("Error initializing SDK: %s", err)
	}

	user, err := deffab.NewUser(sdk.Config, sdk.MSPClient, "admin", "adminpw", setup.OrgID)
	if err != nil {
		return fmt.Errorf("NewUser returned error: %v", err)
	}
	err = sdk.FabricClient.SaveUserToStateStore(user, false)
	if err != nil {
		return fmt.Errorf("client.SaveUserToStateStore returned error: %v", err)
	}

	sdk.FabricClient.SetUserContext(user)

	setup.Client = sdk.FabricClient

	org1Admin, err := GetAdmin(sdk.FabricClient, "org1", setup.OrgID)
	if err != nil {
		return fmt.Errorf("Error getting org admin user: %v", err)
	}

	org1User, err := GetUser(sdk.FabricClient, "org1", setup.OrgID)
	if err != nil {
	...
	ordererAdmin, err := GetOrdererAdmin(sdk.FabricClient, setup.OrgID)
	if err != nil {
		return fmt.Errorf("Error getting orderer admin user: %v", err)
	}

	// Check if primary peer has joined channel
	alreadyJoined, err := HasPrimaryPeerJoinedChannel(sdk.FabricClient, org1Admin, channel)
	if err != nil {
	...
	if !alreadyJoined {
		...
		if err = admin.CreateOrUpdateChannel(sdk.FabricClient, ordererAdmin, org1Admin, channel, setup.ChannelConfig); err != nil {
			return fmt.Errorf("CreateChannel returned error: %v", err)
		}
		time.Sleep(time.Second * 3)

		sdk.FabricClient.SetUserContext(org1Admin)
		if err = channel.Initialize(nil); err != nil {
			return fmt.Errorf("Error initializing channel: %v", err)
		}

		if err = admin.JoinChannel(sdk.FabricClient, org1Admin, channel); err != nil {
	...
	sdk.FabricClient.SetUserContext(org1User)

	if err := setup.setupEventHub(sdk.FabricClient); err != nil {
		return err
	}
