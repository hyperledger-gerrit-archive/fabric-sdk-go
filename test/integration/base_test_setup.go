...
func (setup *BaseSetupImpl) Initialize(sdk *fabsdk.FabricSDK) error {
	...
	req := resmgmt.SaveChannelRequest{ChannelID: setup.ChannelID, ChannelConfigTxReader: r, SigningIdentities: []msp.SigningIdentity{adminIdentity}}
...
func CreateChannelAndUpdateAnchorPeers(t *testing.T, sdk *fabsdk.FabricSDK, channelID string, channelConfigFile string, orgsContext []*OrgContext) error {
	...
	req := resmgmt.SaveChannelRequest{
		ChannelID:           channelID,
		ChannelConfigTxPath: GetChannelConfigTxPath(channelConfigFile),
	...
	for _, orgCtx := range orgsContext {
		req := resmgmt.SaveChannelRequest{
			ChannelID:           channelID,
			ChannelConfigTxPath: GetChannelConfigTxPath(orgCtx.AnchorPeerConfigFile),
