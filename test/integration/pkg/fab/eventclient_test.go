...
func TestMultipleEventsBySeekTypes(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		assert.True(t, txIdMatched, "TxID from one of the event didn't match with test transaction TxID")
	}
	...
	assert.False(t, txIdMatched, "TxID from each event matched with TxID of transaction after each registration, which isn't conventional seektype NEWEST behavior")
}
