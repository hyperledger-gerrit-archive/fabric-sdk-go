...
func TestQueryCollectionsConfig(t *testing.T) {
	...
	case *cb.CollectionConfig_StaticCollectionConfig:
		...
		if cconf.StaticCollectionConfig.BlockToLive != 1000 {
			t.Fatalf("The property of BlockToLive is incorrect, expected 1000, got %d", cconf.StaticCollectionConfig.BlockToLive)
		}
		if cconf.StaticCollectionConfig.RequiredPeerCount != 0 {
			t.Fatalf("The property of RequiredPeerCount is incorrect, expected 0, got %d", cconf.StaticCollectionConfig.RequiredPeerCount)
		}
		if cconf.StaticCollectionConfig.MaximumPeerCount != 2 {
			t.Fatalf("The property of MaximumPeerCount is incorrect, expected 2, got %d", cconf.StaticCollectionConfig.MaximumPeerCount)
		}
		if cconf.StaticCollectionConfig.MemberOrgsPolicy.GetSignaturePolicy() == nil {
			t.Fatalf("The property of MemberOrgsPolicy shouldn't be nil")
		}
	default:
