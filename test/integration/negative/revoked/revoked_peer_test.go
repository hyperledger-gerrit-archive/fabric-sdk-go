...
func testRevokedPeer(t *testing.T) {
	...
	sdk1, err := fabsdk.New(getConfigBackend())
	require.NoError(t, err)
	defer sdk1.Close()
...
func createCC(t *testing.T, org1ResMgmt *resmgmt.Client, org2ResMgmt *resmgmt.Client) {
	...
	require.Equalf(t, stat.Code, int32(status.SignatureVerificationFailed), "Expecting signature verification error due to revoked cert", err)
