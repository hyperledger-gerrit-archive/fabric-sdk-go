...
func TestOrgsEndToEnd(t *testing.T) {
	...
	txOpts := []apitxn.Option{integration.WithProposalProcessor([]apitxn.ProposalProcessor{orgTestPeer1})}

	_, _, err = chClientOrg2User.ExecuteTx(apitxn.Request{ChaincodeID: "exampleCC", Fcn: "invoke", Args: integration.ExampleCCTxArgs()}, txOpts...)
	if err != nil {
	...
	txOpts = []apitxn.Option{integration.WithProposalProcessor([]apitxn.ProposalProcessor{orgTestPeer0, orgTestPeer1})}
	_, _, err = chClientOrg2User.ExecuteTx(apitxn.Request{ChaincodeID: "exampleCC", Fcn: "invoke", Args: integration.ExampleCCTxArgs()}, txOpts...)
	if err != nil {
		t.Fatalf("Failed to move funds: %s", err)
	}
	...
	_, _, err = chClientOrg2User.ExecuteTx(apitxn.Request{ChaincodeID: "exampleCC", Fcn: "invoke", Args: integration.ExampleCCTxArgs()})
	if err != nil {
		t.Fatalf("Failed to move funds: %s", err)
	}
...
func verifyValue(t *testing.T, chClient apitxn.ChannelClient, expected int) {
	...
	for i := 0; i < pollRetries; i++ {
		...
		queryOpts := []apitxn.Option{integration.WithProposalProcessor([]apitxn.ProposalProcessor{orgTestPeer0})}
		value, err := chClient.Query(apitxn.Request{ChaincodeID: "exampleCC", Fcn: "invoke", Args: integration.ExampleCCQueryArgs()}, queryOpts...)
		if err != nil {
