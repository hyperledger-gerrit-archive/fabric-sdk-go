...
func TestOrgsEndToEnd(t *testing.T) {
	...
	sdk, err := fabsdk.New(
		fabsdk.WithConfig(config.FromFile("../" + integration.ConfigTestFile)))
	if err != nil {
	...
	_, _, err = chClientOrg2User.ExecuteTx(apitxn.ExecuteTxRequest{ChaincodeID: "exampleCC", Fcn: "invoke", Args: integration.ExampleCCTxArgs()})
	if err != nil {
		t.Fatalf("Failed to move funds: %s", err)
	}
