...
func TestOrgsEndToEnd(t *testing.T) {
	...
	_, _, err = chClientOrg2User.ExecuteTx(apitxn.Request{ChaincodeID: "exampleCC", Fcn: "invoke", Args: integration.ExampleCCTxArgs()}, apitxn.WithProposalProcessor(orgTestPeer1))
	if err != nil {
	...
	_, _, err = chClientOrg2User.ExecuteTx(apitxn.Request{ChaincodeID: "exampleCC", Fcn: "invoke", Args: integration.ExampleCCTxArgs()}, apitxn.WithProposalProcessor(orgTestPeer0, orgTestPeer1))
	if err != nil {
		t.Fatalf("Failed to move funds: %s", err)
	}
