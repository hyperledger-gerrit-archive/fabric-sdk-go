...
func initializeFabricClient(t *testing.T) {
	...
	err = factory.InitFactories(configImpl.GetCSPConfig())
	failTestIfError(err, t)
...
func installAndInstantiate(t *testing.T) {
	...
	err = admin.SendInstantiateCC(orgTestChannel, "exampleCC", orgTestChannel.GetName(),
		generateInitArgs(), "github.com/example_cc", "0", []api.Peer{orgTestPeer1}, peer1EventHub)
	failTestIfError(err, t)
}

func loadOrderer(t *testing.T) {
	ordererConfig, err := orgTestClient.GetConfig().GetRandomOrdererConfig()
	failTestIfError(err, t)
...
func loadOrgPeers(t *testing.T) {
	org1Peers, err := orgTestClient.GetConfig().GetPeersConfig(org1)
	failTestIfError(err, t)

	org2Peers, err := orgTestClient.GetConfig().GetPeersConfig(org2)
	failTestIfError(err, t)
...
func loadOrgUsers(t *testing.T) {
	...
	ordererAdminUser, err = integration.GetOrdererAdmin(orgTestClient, "ordererOrg")
	failTestIfError(err, t)
