...
func testDistributedSignatures(t *testing.T, examplecc string) {
	...
	// to generate a signature from the SDK, uncomment the following lines and comment out the line right after this comment
	/*org1DsChannelCfgSigSDK := createSignature(t, org1ClCtx, chConfigPath, adminUser)
	f, err := os.Create(fmt.Sprintf("%s/signatures/%s_%s_%s_sbytes.txt_SDK.sha256", signatureWD, dsChannel, org1ClCtx.org, adminUser))
	require.NoError(t, err, "Failed to create temporary file")
	defer f.Close()
	bufferedWriter := bufio.NewWriter(f)
	_, err = bufferedWriter.Write(org1DsChannelCfgSigSDK.Signature)
	assert.NoError(t, err, "must be able to write signature of [%s-%s] to buffer", org1ClCtx.org, adminUser)
	bufferedWriter.Flush()
	shf, err := os.Create(fmt.Sprintf("%s/%s_%s_%s_sHeaderbytes_SDK.txt", signatureWD, dsChannel, org1ClCtx.org, adminUser))
	require.NoError(t, err, "Failed to create temporary file")
	defer shf.Close()
	bufferedWriter = bufio.NewWriter(shf)
	_, err = bufferedWriter.Write(org1DsChannelCfgSigSDK.SignatureHeader)
	assert.NoError(t, err, "must be able to write signature header of [%s-%s] to buffer", org1ClCtx.org, adminUser)
	bufferedWriter.Flush()
	org1DsChannelCfgSig := org1DsChannelCfgSigSDK
	*/
	org1DsChannelCfgSig := executeExternalSigning(t, org1ClCtx, chConfigPath, adminUser, sigDir)
	t.Logf("org1DsChannelCfgSig:[%+v]", org1DsChannelCfgSig)

	// create org2 ConfigSignature
	/*org2DsChannelCfgSigSDK := createSignature(t, org2ClCtx, chConfigPath, adminUser)
	f2, err := os.Create(fmt.Sprintf("%s/signatures/%s_%s_%s_sbytes.txt_SDK.sha256", signatureWD, dsChannel, org2ClCtx.org, adminUser))
	require.NoError(t, err, "Failed to create temporary file")
	defer f2.Close()

	bufferedWriter = bufio.NewWriter(f2)
	_, err = bufferedWriter.Write(org2DsChannelCfgSigSDK.Signature)
	assert.NoError(t, err, "must be able to write signature of [%s-%s] to buffer", org2ClCtx.org, adminUser)
	bufferedWriter.Flush()
	shf2, err := os.Create(fmt.Sprintf("%s/%s_%s_%s_sHeaderbytes_SDK.txt", signatureWD, dsChannel, org2ClCtx.org, adminUser))
	require.NoError(t, err, "Failed to create temporary file")
	defer shf2.Close()
	bufferedWriter = bufio.NewWriter(shf2)
	_, err = bufferedWriter.Write(org2DsChannelCfgSigSDK.SignatureHeader)
	assert.NoError(t, err, "must be able to write signature header of [%s-%s] to buffer", org2ClCtx.org, adminUser)
	bufferedWriter.Flush()
	org2DsChannelCfgSig := org2DsChannelCfgSigSDK
	*/
	org2DsChannelCfgSig := executeExternalSigning(t, org2ClCtx, chConfigPath, adminUser, sigDir)
	...
	/*org1MSPDsChannelCfgSigSDK := createSignature(t, org1ClCtx, chConfigOrg1MSPPath, adminUser)
	f, err = os.Create(fmt.Sprintf("%s/signatures/%s%sMSPanchors_%s_%s_sbytes.txt_SDK.sha256", signatureWD, dsChannel, org1ClCtx.org, org1ClCtx.org, adminUser))
	require.NoError(t, err, "Failed to create temporary file")
	defer f.Close()
	bufferedWriter = bufio.NewWriter(f)
	_, err = bufferedWriter.Write(org1MSPDsChannelCfgSigSDK.Signature)
	assert.NoError(t, err, "must be able to write signature of [%s-%s] to buffer", org1ClCtx.org, adminUser)
	bufferedWriter.Flush()
	shf, err = os.Create(fmt.Sprintf("%s/%s%sMSPanchors_%s_%s_sHeaderbytes_SDK.txt", signatureWD, dsChannel, org1ClCtx.org, org1ClCtx.org, adminUser))
	require.NoError(t, err, "Failed to create temporary file")
	defer shf.Close()
	bufferedWriter = bufio.NewWriter(shf)
	_, err = bufferedWriter.Write(org1MSPDsChannelCfgSigSDK.SignatureHeader)
	assert.NoError(t, err, "must be able to write signature header of [%s-%s] to buffer", org1ClCtx.org, adminUser)
	bufferedWriter.Flush()
	org1MSPDsChannelCfgSig := org1MSPDsChannelCfgSigSDK
	*/
	org1MSPDsChannelCfgSig := executeExternalSigning(t, org1ClCtx, chConfigOrg1MSPPath, adminUser, sigDir)
	...
	/*org2MSPDsChannelCfgSigSDK := createSignature(t, org2ClCtx, chConfigOrg2MSPPath, adminUser)
	f, err = os.Create(fmt.Sprintf("%s/signatures/%s%sMSPanchors_%s_%s_sbytes.txt_SDK.sha256", signatureWD, dsChannel, org2ClCtx.org, org2ClCtx.org, adminUser))
	require.NoError(t, err, "Failed to create temporary file")
	defer f.Close()
	bufferedWriter = bufio.NewWriter(f)
	_, err = bufferedWriter.Write(org2MSPDsChannelCfgSigSDK.Signature)
	assert.NoError(t, err, "must be able to write signature of [%s-%s] to buffer", org2ClCtx.org, adminUser)
	bufferedWriter.Flush()
	shf, err = os.Create(fmt.Sprintf("%s/%s%sMSPanchors_%s_%s_sHeaderbytes_SDK.txt", signatureWD, dsChannel, org2ClCtx.org, org2ClCtx.org, adminUser))
	require.NoError(t, err, "Failed to create temporary file")
	defer shf.Close()
	bufferedWriter = bufio.NewWriter(shf)
	_, err = bufferedWriter.Write(org2MSPDsChannelCfgSigSDK.SignatureHeader)
	assert.NoError(t, err, "must be able to write signature header of [%s-%s] to buffer", org2ClCtx.org, adminUser)
	bufferedWriter.Flush()
	org2MSPDsChannelCfgSig := org2MSPDsChannelCfgSigSDK
	*/
	org2MSPDsChannelCfgSig := executeExternalSigning(t, org2ClCtx, chConfigOrg2MSPPath, adminUser, sigDir)
