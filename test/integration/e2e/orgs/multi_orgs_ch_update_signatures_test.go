...
func testDistributedSignatures(t *testing.T, examplecc string) {
	...
	/*	org1Peers, err := integration.DiscoverLocalPeers(org1Ctx, 2)
		...
		require.NoError(t, err)
	*/
	//TODO remove below two lines and uncomment above peer discovery code once fixed (currently DiscoveryLocalPeers() returns an empty list).
...
func createDSClientCtx(t *testing.T, org string) *dsClientCtx {
	...
	configProvider := config.FromFile(integration.GetConfigPath(cfgFile))
	...
	var err error
	d.sdk, err = fabsdk.New(configProvider)
