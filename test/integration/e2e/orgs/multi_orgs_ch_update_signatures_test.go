...
// testDistributedSignatures will create at least 2 clients, each from 2 different orgs and creates two channel where these 2 orgs are members
...
func testDistributedSignatures(t *testing.T, examplecc string) {
	...
	// use OpenSSL signing
	// TODO investigate OpenSSL signature, why it is refused by the orderer..
...
func e2eCreateAndQueryChannel(t *testing.T, ordererClCtx, org1ClCtx, org2ClCtx *dsClientCtx, channelID, examplecc string) {
	...
	t.Logf("created tempDir for sdk signatures: %s", sigDir)
...
func createDSClientCtx(t *testing.T, org string) *dsClientCtx {
	...
	d.sdk, err = fabsdk.New(b, fabsdk.WithServicePkg(&DynamicDiscoveryProviderFactory{}))
