...
func DistributedSignaturesTests(t *testing.T, examplecc string) {
	...
	// use OpenSSL signing
	// TODO investigate OpenSSL signature, why it is sometimes refused by the orderer..
...
func e2eCreateAndQueryChannel(t *testing.T, ordererClCtx, org1ClCtx, org2ClCtx *dsClientCtx, channelID, examplecc string) {
	...
	sigs := generateSignatures(t, org1ClCtx, org2ClCtx, chConfigPath, chConfigOrg1MSPPath, chConfigOrg2MSPPath, sigDir, (channelID == dsChannelSDK))
...
func loadExternalSignature(t *testing.T, org, chConfigPath, user, sigDir string) *common.ConfigSignature {
	...
	fName := fmt.Sprintf("%s/%s_%s_%s_sbytes.txt.sha256", sigDir, chCfgName, org, user)
	...
	fName = fmt.Sprintf("%s/%s_%s_%s_sHeaderbytes.txt", sigDir, chCfgName, org, user)
	...
	if strings.Contains(sigDir, "ext") {
		key := loadOrgUserPrivateKey(t, org, user)
		newSig, err := utils.SignatureToLowS(&key.(*ecdsa.PrivateKey).PublicKey, sig)
		require.NoError(t, err, "failed to switch signature to LowS")
		sig = newSig
	}
...
type endpointConfigEntity struct {
	...
	OrganizationConfig map[string]fabImpl.OrganizationConfig
}
...
func loadOrgUserPrivateKey(t *testing.T, org, user string) interface{} {
	...
	pathToKey := fmt.Sprintf("peerOrganizations/%s.example.com/users/%s@%s.example.com/msp/keystore", o, user, o)
