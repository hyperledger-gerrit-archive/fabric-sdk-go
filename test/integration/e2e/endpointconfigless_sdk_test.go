...
// endpointconfigless_sdk_test.go is an example of programmatically configuring the sdk by injecting instances that implement EndpointConfig's functions (representing the sdk's configs)
...
var (
	...
	networkConfig = fab.NetworkConfig{
		...
		Peers:         peersConfig,
		CertificateAuthorities: caConfig,
		// TODO test with entityMatchers as well
	...
	orderersConfigImpl   = &exampleOrderersConfig{}
	ordererConfigImpl    = &exampleOrdererConfig{}
	peersConfigImpl      = &examplePeersConfig{}
...
func (m *exampleTimeout) Timeout(tType fab.TimeoutType) time.Duration {
	...
	s.set(nil, func() bool { return tType == fab.EndorserConnection }, func() { timeout = time.Second * 10 })
	s.set(nil, func() bool { return tType == fab.PeerResponse }, func() { timeout = time.Minute * 3 })
	s.set(nil, func() bool { return tType == fab.DiscoveryGreylistExpiry }, func() { timeout = time.Second * 10 })
	s.set(nil, func() bool { return tType == fab.EventHubConnection }, func() { timeout = time.Second * 15 })
	s.set(nil, func() bool { return tType == fab.EventReg }, func() { timeout = time.Second * 15 })
	s.set(nil, func() bool { return tType == fab.OrdererConnection }, func() { timeout = time.Second * 15 })
	s.set(nil, func() bool { return tType == fab.OrdererResponse }, func() { timeout = time.Minute * 2 })
	s.set(nil, func() bool { return tType == fab.DiscoveryConnection }, func() { timeout = time.Second * 15 })
	s.set(nil, func() bool { return tType == fab.DiscoveryResponse }, func() { timeout = time.Second * 15 })
	s.set(nil, func() bool { return tType == fab.Query }, func() { timeout = time.Minute * 3 })
	s.set(nil, func() bool { return tType == fab.Execute }, func() { timeout = time.Minute * 3 })
	s.set(nil, func() bool { return tType == fab.ResMgmt }, func() { timeout = time.Minute * 3 })
	s.set(nil, func() bool { return tType == fab.ConnectionIdle }, func() { timeout = time.Second * 30 })
	s.set(nil, func() bool { return tType == fab.EventServiceIdle }, func() { timeout = time.Minute * 2 })
	s.set(nil, func() bool { return tType == fab.ChannelConfigRefresh }, func() { timeout = time.Minute * 90 })
	s.set(nil, func() bool { return tType == fab.ChannelMembershipRefresh }, func() { timeout = time.Second * 60 })
	s.set(nil, func() bool { return tType == fab.DiscoveryServiceRefresh }, func() { timeout = time.Second * 10 })
	// EXPERIMENTAL - do we need this to be configurable?
	s.set(nil, func() bool { return tType == fab.CacheSweepInterval }, func() { timeout = time.Second * 15 })
...
func (m *examplePeerMSPID) PeerMSPID(peerNameOrURL string) (string, error) {
	var mspID string
	...
	for _, org := range orgsConfig {
		for i := 0; i < len(org.Peers); i++ {
			if strings.EqualFold(org.Peers[i], peerNameOrURL) {
				...
				mspID = org.MSPID
				break
				// TODO find peer matcher once implemented in NetworkConfig()
				//} else {
				//
...
func (m *exampleOrdererConfig) OrdererConfig(ordererNameOrURL string) (*fab.OrdererConfig, error) {
	...
	if !ok {
		// TODO enable entityMatchers and uncomment code below
...
func (m *examplePeerConfig) PeerConfig(nameOrURL string) (*fab.PeerConfig, error) {
	...
	// TODO add with entityMatchers once implemented in NetworkConfig()
...
func (m *exampleEventServiceType) EventServiceType() fab.EventServiceType {
	...
	//or for EventHub service type, but most configs use Delivery
