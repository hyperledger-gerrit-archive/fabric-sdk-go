...
func createChannelAndCC(t *testing.T, sdk *fabsdk.FabricSDK) {
	...
	if err = orgResMgmt.JoinChannel(channelID, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithNetworkOrderer("orderer.example.com")); err != nil {
...
func createChannel(sdk *fabsdk.FabricSDK, t *testing.T, resMgmtClient *resmgmt.Client) {
	...
	txID, err := resMgmtClient.SaveChannel(req, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithNetworkOrderer("orderer.example.com"))
	require.Nil(t, err, "error should be nil")
