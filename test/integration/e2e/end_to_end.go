...
func Run(t *testing.T, configOpt core.ConfigProvider, sdkOpts ...fabsdk.Option) {
	...
	assert.EqualValues(t, status.ChaincodeGRPCError, s.Code, "expected ChaincodeGRPCError")
	assert.True(t, len(s.Details) > 0, "expected Details to exist in ChaincodeGRPCError")
	grpcStatus := s.Details[0].(*status.GRPCStatus)
	if grpcStatus.Code != 500 {
		t.Fatalf("Expected 500 grpc status code in ChaincodeGRPCError")
	}
