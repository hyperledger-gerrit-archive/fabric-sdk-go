...
func testQueryWithOpts(expected string, ccID string, chClient apitxn.ChannelClient, t *testing.T) {
	...
	queryOpts := []apitxn.Option{integration.WithNotifier(notifier)}
	result, err := chClient.Query(apitxn.Request{ChaincodeID: ccID, Fcn: "invoke", Args: integration.ExampleCCQueryArgs()}, queryOpts...)
	if err != nil {
...
func testAsyncTransaction(ccID string, chClient apitxn.ChannelClient, t *testing.T) {
	...
	txOpts := []apitxn.Option{integration.WithNotifier(txNotifier)}
	_, _, err := chClient.ExecuteTx(apitxn.Request{ChaincodeID: ccID, Fcn: "invoke", Args: integration.ExampleCCTxArgs()}, txOpts...)
	if err != nil {
		t.Fatalf("Failed to move funds: %s", err)
	}
...
func testChaincodeEvent(ccID string, chClient apitxn.ChannelClient, t *testing.T) {
	...
	// Synchronous transaction
	_, txID, err := chClient.ExecuteTx(apitxn.Request{ChaincodeID: ccID, Fcn: "invoke", Args: integration.ExampleCCTxArgs()})
	if err != nil {
		t.Fatalf("Failed to move funds: %s", err)
	}
