...
func TestLedgerClientQueries(t *testing.T) {
	...
	expectedPeerConfig1, err := endpointConfig.PeerConfig("peer0.org1.example.com")
	require.NoErrorf(t, err, "Unable to fetch Peer config for %s", "peer0.org1.example.com")
	expectedPeerConfig2, err := endpointConfig.PeerConfig("peer1.org1.example.com")
	require.NoErrorf(t, err, "Unable to fetch Peer config for %s", "peer1.org1.example.com")
