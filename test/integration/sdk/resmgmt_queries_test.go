...
func TestResMgmtClientQueries(t *testing.T) {
	...
	testQueryConfig(t, testSetup.ChannelID, client)
...
func testQueryConfig(t *testing.T, channelID string, client *resmgmt.Client) {

	channelCfg, err := client.QueryConfig(channelID)
	if err != nil {
	...
	channelCfg, err = client.QueryConfig(channelID, resmgmt.WithOrdererID("orderer.example.com"))
	if err != nil {
		t.Fatalf("QueryConfig return error: %v", err)
	}
	if !contains(channelCfg.Orderers(), expected) {
	...
	channelCfg, err = client.QueryConfig(channelID, resmgmt.WithOrdererID("non-existent"))
	if err == nil {
