...
func TestGetAllIdentities(t *testing.T) {
	...
	user1 := integration.GenerateRandomID()
	user2 := integration.GenerateRandomID()
	...
	_, err := mspClient.CreateIdentity(req)
	if err != nil {
		t.Fatalf("Create identity failed: %v", err)
	}
	...
	if len(identities) != 2 {
		t.Fatalf("Expected 2, got %d identities", len(identities))
	}
