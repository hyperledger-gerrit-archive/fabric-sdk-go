...
func testEndorsers(t *testing.T, orgsContext []*integration.OrgContext, interest *fabdiscovery.ChaincodeInterest, expectedEndorserGroups ...[]string) {
	ctxProvider := mainSDK.Context(fabsdk.WithUser(org1User), fabsdk.WithOrg(org1Name))
	ctx, err := ctxProvider()
	require.NoError(t, err, "error getting channel context")
