...
func TestExpiredCert(t *testing.T) {
	...
	sdk, err := fabsdk.New(config.FromFile("../../fixtures/config/config_expired_test.yaml"))
	if err != nil {
	...
	//error in GRPC log is ' Failed to dial orderer.example.com:7050: connection error: desc = "transport: authentication handshake failed: x509: certificate has expired or is not yet valid"; '
	if err == nil {
