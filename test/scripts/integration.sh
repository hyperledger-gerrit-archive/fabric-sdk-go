...
PKGS=`$GO_CMD list $REPO/test/integration/orgs/... 2> /dev/null | \
...
$GO_CMD test -run TestOrgsEndToEnd -tags "$GO_TAGS" $GO_TESTFLAGS -ldflags="$GO_LDFLAGS" $PKGS -v -timeout=40m -count=1 configFile=${CONFIG_FILE} testLocal=${TEST_LOCAL}
