...
GO_METALINTER_PKG="gopkg.in/alecthomas/gometalinter.v2"
GOPATH="${GOPATH:-${HOME}/go}"

DEPEND_SCRIPT_REVISION=$(git log -1 --pretty=format:"%h" test/scripts/dependencies.sh)
DATE=$(date +"%m-%d-%Y")

LASTRUN_INFO_PATH="/tmp/fabricsdkgo-depend-run.txt"
...
function installGoMetalinter {
    ...
    GOPATH=${BUILD_TMP} ${GO_CMD} get -u ${GO_METALINTER_PKG}

    mkdir -p ${GOPATH}/bin
    cp ${BUILD_TMP}/bin/* ${GOPATH}/bin
    rm -Rf ${GOPATH}/src/${GO_METALINTER_PKG}
    mkdir -p ${GOPATH}/src/${GO_METALINTER_PKG}
    cp -Rf ${BUILD_TMP}/src/${GO_METALINTER_PKG}/* ${GOPATH}/src/${GO_METALINTER_PKG}
...
type ${GO_METALINTER_CMD} >/dev/null 2>&1 || { echo >& 2 "gometalinter is not installed (go get -u ${GO_METALINTER_PKG})"; ABORT=1; }
