...
if [ "$TEST_CHANGED_ONLY" = true ]; then
    # findChangedPackages assumes that the working directory contains the repo; so change to the repo directory.
    PWD=$(pwd)
    cd "${GOPATH}/src/${REPO}"
...
if [ "$TEST_RACE_CONDITIONS" = true ]; then
    ...
    if [ "$ARCH" == "x86_64" ]; then
        echo "Enabling race condition flag for upcoming PKCS11 unit test run"
        RACEFLAG="-race"
    else
        echo "Race condition flag not supported on $ARCH for upcoming PKCS11 unit test run"
    fi
fi
