...
function isLastPopulateCurrent {
    ...
    declare cachedVendorTree=$(cat < "${CACHE_PATH}/${VENDOR_TREE_FILENAME}")

    if [[ "${filesModified}" =~ ( |^)(test/scripts/populate-vendor.sh|Gopkg.lock)($'\n'|$) ]]; then
        echo "Vendor script or Gopkg.lock modified - repopulating vendor"
        return 1
    fi

    if [ "${VENDOR_TREE}" != "${cachedVendorTree}" ]; then
        echo "Vendor directory modified - repopulating vendor"
        return 1
    fi
