...
LOCK_REVISION=$(git log -1 --pretty=format:"%h" go.mod)
DATE=$(date +"%m-%d-%Y")
...
function isScriptCurrent {
    ...
    declare matcher='( |^)(test/scripts/populate-vendor.sh|go.mod)( |$)'
    if [[ "${filesModified}" =~ ${matcher} ]]; then
        echo "Vendor script or go.mod modified - will need to repopulate vendor"
        return 1
    fi
}
...
function isLastPopulateCurrent {
    ...
    if [ -f "${CACHE_PATH}/${LASTRUN_INFO_FILENAME}" ]; then
        ...
        echo "Last populated vendor on ${lastScriptUsage[2]} using revision ${lastScriptUsage[0]} with go.mod revision ${lastScriptUsage[1]}"
...
function populateVendor {
	...
	GO111MODULE=on ${GO_CMD} mod vendor
    ...
    git clone --branch release-1.4 --depth=1 https://github.com/hyperledger/fabric.git ${chaincodedPath}/vendor/github.com/hyperledger/fabric
